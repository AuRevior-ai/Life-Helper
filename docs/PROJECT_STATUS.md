...
阶段 26：真实环境基础验证已完成，作为 checkpoint 保留。本阶段只固化人工验收结果，不修改业务代码。

阶段 24F：管理员端会员 / 优惠券 / 营销配置页面 UI 收口已完成基础版，作为 checkpoint 保留。本阶段只修改页面 UI、结构保护测试和阶段文档，不修改云函数、service、schema 或状态机。
...
| ---- | ---- |
| 云函数上传 | 成功 |
| 开发者工具基础体验 | 通过，未发现明显 bug |
| 真机基础体验 | 通过，未发现明显 bug |
| 真实云环境基础验证 | 通过 |

本轮仅固化文档，未修改业务代码，未生成公开交付候选目录。

阶段 24F 管理员端会员 / 优惠券 / 营销配置页面 UI 收口验收记录：

| 命令 | 结果 |
| ---- | ---- |
| `node --test tests/phase24f_admin_marketing_ui.test.js` | 通过，8/8 |
| `npm test` | 通过，349/349 |
| `git diff --check` | 通过 |
...
| P2 | 订单、资质、保证金列表真实云索引后续仍需随数据量增长复核 | 阶段 24E 已将用户/师傅订单列表、管理员资质列表、管理员保证金列表改为仓库侧分页；阶段 26 人工真实环境基础验证已通过 |
...
| P1 | 管理员端二级页面仍需真机复核或少量剩余运营页收口 | 阶段 24B-1、24B-2、24B-3 已分批收口主要管理员二级页；阶段 24F 已收口会员、优惠券、营销配置页面，后续可做全量真机视觉抽查 |
...
1. 建议进入 clean candidate 交付包整理；生成候选目录后必须运行 release-risk 扫描。
2. 或继续做管理员端剩余运营配置页面真机视觉抽查与小范围体验修正。
...
- 阶段 24F 记录：`docs/dev-records/24f-admin-marketing-ui.md`
- 阶段 26 记录：`docs/dev-records/26-real-env-validation.md`
