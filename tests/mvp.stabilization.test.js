...
test('profile page differentiates user, worker, and admin roles with dedicated actions', () => {
  ...
  assert.match(profileJs, /roleHomeTitle/)
  assert.match(profileJs, /claimInitialAdmin/)
  assert.match(profileJs, /居民中心/)
  assert.match(profileJs, /师傅工作台/)
  assert.match(profileJs, /管理工作台/)
  assert.match(profileWxml, /完善资料/)
  assert.match(profileWxss, /role-card\.worker/)
  assert.match(profileWxss, /role-card\.admin/)
})
