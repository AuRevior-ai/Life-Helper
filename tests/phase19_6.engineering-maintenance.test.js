...
test('release risk scanner ignores docs, tests, mock, and example payment references', () => {
  ...
  fs.mkdirSync(path.join(tempDir, 'cloudfunctions', 'payment'), { recursive: true })
  fs.writeFileSync(path.join(tempDir, 'docs', 'wechat-pay-setup.md'), 'mchid apiv3 apiclient_key.pem setup note')
  fs.writeFileSync(path.join(tempDir, 'docs', 'wechat-pay-config.example.md'), 'example APIv3')
  fs.writeFileSync(path.join(tempDir, 'tests', 'phase13.wechat-pay-lite.test.js'), 'assert.match(text, /mchid/)')
  fs.writeFileSync(path.join(tempDir, 'cloudfunctions', 'payment', 'config.example.js'), 'module.exports = { mchid: "example" }')
  fs.writeFileSync(path.join(tempDir, 'cloudfunctions', 'payment', 'wechat-pay-client.js'), 'mock placeholder client')
...
test('release risk scanner blocks sensitive delivery files', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'release-risk-sensitive-'))
  fs.writeFileSync(path.join(tempDir, '.env'), 'SECRET=1')
  fs.writeFileSync(path.join(tempDir, 'project.private.config.json'), '{}')
  fs.writeFileSync(path.join(tempDir, 'apiclient_key.pem'), '-----BEGIN PRIVATE KEY-----\nsecret')
...
test('README AppID guidance follows current config without hardcoding a historical AppID in tests', () => {
  ...
  assert.ok(projectConfig.appid, 'project.config.json should define the current appid strategy')
  ...
  assert.doesNotMatch(readme, new RegExp(`当前 \`appid\` 使用 \`${projectConfig.appid}\``))
})

test('PAY_MODE=wechat fails fast instead of returning empty real payment parameters', async () => {
  const { createWechatPayClient } = require('../cloudfunctions/payment/wechat-pay-client')
  const client = createWechatPayClient({ enabled: true })

  await assert.rejects(
    () => client.createPrepay({ out_trade_no: 'order_1' }),
    (error) => {
      assert.equal(error.errorCode || error.message, 'WECHAT_PAY_NOT_IMPLEMENTED')
...
test('more cloudfunctions consume shared low-risk utilities', () => {
  const targets = ['order', 'admin', 'finance', 'refund', 'review', 'worker', 'promotion', 'dispatch', 'area', 'service', 'user', 'login']
  const wired = targets.filter((name) => {
    const handler = read(path.join('cloudfunctions', name, 'handler.js'))
    return /require\(['"]\.\/_shared\/(?:response|payload|time|pagination)['"]\)/.test(handler)
  })

  assert.ok(wired.length >= 7, `expected at least 7 phase 19.6 shared utility consumers, got ${wired.join(', ')}`)
})

test('cloudfunction wx-server-sdk dependency versions stay unified', () => {
  const versions = new Map()
  for (const entry of fs.readdirSync(path.join(rootDir, 'cloudfunctions'), { withFileTypes: true })) {
  ...
  assert.equal(versions.size, 1, JSON.stringify(Object.fromEntries(versions), null, 2))
})
...
test('core database JSON schemas exist and are referenced by markdown contract', () => {
  ...
  for (const schemaFile of schemaFiles) {
    ...
    for (const key of [
      ...
      assert.ok(Object.prototype.hasOwnProperty.call(schema, key), `${schemaFile} missing ${key}`)
    }
    assert.ok(schema.requiredFields.length > 0, `${schemaFile} should have requiredFields`)
    assert.match(markdown, new RegExp(schemaFile.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
  }
})
...
test('api action manifest covers handlers and is referenced by markdown contract', () => {
  ...
  for (const functionName of ['order', 'payment', 'merchant', 'admin', 'finance', 'refund', 'review', 'worker', 'promotion', 'dispatch', 'area', 'service', 'user', 'login', 'message', 'address', 'tip']) {
    assert.ok(manifest[functionName], `${functionName} missing from manifest`)
    assert.ok(Array.isArray(manifest[functionName].actions), `${functionName}.actions should be an array`)
    const handler = read(path.join('cloudfunctions', functionName, 'handler.js'))
    for (const action of manifest[functionName].actions) {
      assert.match(handler, new RegExp(action.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), `${action} should appear in ${functionName}/handler.js`)
    }
  }
...
test('pagination and indexes contract documents list action risks and shared page limits', () => {
  ...
  assert.deepEqual(normalizePage({ page: 0, pageSize: 999 }), { page: 1, pageSize: 50 })
  assert.deepEqual(normalizePage({ page: 'bad', pageSize: 'bad' }), { page: 1, pageSize: 20 })

  for (const snippet of ['listOrders', 'listFinanceLogs', 'listWorkerEarnings', 'listReviews', 'listAfterSales', 'listDispatchLogs', 'listMessages', '最大 pageSize']) {
