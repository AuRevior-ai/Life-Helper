...
test("cloud function transport failures are converted to deployment guidance instead of business not-found copy", async () => {
    ...
    cloud: {
      async callFunction() {
        const error = new Error(
          "cloud.callFunction:fail Error: errCode:-504002 function not found",
        );
  ...
  await assert.rejects(
    () =>
      callCloudFunction("service", "getServiceDetail", {
...
test("service and order pages render cloud failure state separately from missing service state", () => {
  const serviceDetailJs = read(
    "miniprogram/pages/service-detail/service-detail.js",
  );
  const serviceDetailWxml = read(
    "miniprogram/pages/service-detail/service-detail.wxml",
  );
