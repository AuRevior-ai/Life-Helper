...
function listHandlerFiles() {
  return fs.readdirSync(path.join(rootDir, 'cloudfunctions'), { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && exists(path.join('cloudfunctions', entry.name, 'handler.js')))
...
test('shared cloudfunction utilities preserve response, payload, time, and pagination contracts', () => {
  const { success, fail, serviceError } = require('../cloudfunctions/_shared/response')
  const { getPayload } = require('../cloudfunctions/_shared/payload')
  const { getNow } = require('../cloudfunctions/_shared/time')
  const { normalizePage, buildPageResult, paginateList } = require('../cloudfunctions/_shared/pagination')

  assert.deepEqual(success({ id: 1 }), { success: true, data: { id: 1 }, message: 'success' })
  ...
  assert.deepEqual(getPayload({ action: 'x', orderId: 'order_1' }), { orderId: 'order_1' })
  assert.deepEqual(getPayload({ payload: { orderId: 'order_2' }, action: 'x' }), { orderId: 'order_2' })

  const now = new Date('2026-06-02T00:00:00.000Z')
  assert.equal(getNow({ now: () => now }), now)

  assert.deepEqual(normalizePage({ page: '2', pageSize: '2' }), { page: 2, pageSize: 2 })
  assert.deepEqual(buildPageResult(['c', 'd'], { total: 5, page: 2, pageSize: 2 }), {
    list: ['c', 'd'],
    total: 5,
    page: 2,
    pageSize: 2,
  ...
  assert.deepEqual(paginateList(['a', 'b', 'c'], { page: 1, pageSize: 2 }, { listKey: 'messages' }), {
    list: ['a', 'b'],
    messages: ['a', 'b'],
    total: 3,
    page: 1,
...
test('status contract keeps core frontend and cloudfunction status constants aligned', () => {
  ...
  assert.deepEqual(finance.WORKER_EARNING_STATUS, frontend.WORKER_EARNING_STATUS)
  assert.deepEqual(promotion.MEMBER_STATUS, frontend.MEMBER_STATUS)
  assert.deepEqual(merchant.SERVICE_PROVIDER_TYPE, frontend.SERVICE_PROVIDER_TYPE)
...
test('governance contract documents exist and contain non-empty required sections', () => {
  const documents = [
    ['docs/release-checklist.md', ['.git', 'project.private.config.json', 'mock 支付', '真实支付']],
    ['docs/contracts/status-contract.md', ['ORDER_STATUS', 'PAY_STATUS', 'REFUND_STATUS', 'MERCHANT_STATUS']],
    ['docs/contracts/database-schema.md', ['users', 'orders', 'merchant_services', 'worker_earnings']],
    ['docs/contracts/api-actions.md', ['order', 'createOrder', 'merchant', 'applyMerchant']],
    ['docs/contracts/permission-matrix.md', ['游客', '普通用户', '师傅', '商家', '管理员']],
    ['docs/dev-records/19.5-engineering-governance.md', ['工程治理', '公共工具', '测试结果', '阶段 20']]
  ]
  ...
  for (const [relativePath, requiredSnippets] of documents) {
    ...
    assert.ok(content.length > 1200, `${relativePath} should not be an empty placeholder`)
    for (const snippet of requiredSnippets) {
      assert.match(content, new RegExp(snippet.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), `${relativePath} should mention ${snippet}`)
    }
  }
})
...
test('release boundary configuration excludes local, secret, and generated delivery risks', () => {
  ...
  for (const pattern of requiredIgnorePatterns) {
    assert.match(gitignore, new RegExp(pattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), `.gitignore missing ${pattern}`)
  }

  for (const forbidden of ['.git', 'project.private.config.json', 'APIv3', '证书', '真实支付配置']) {
    assert.match(releaseChecklist, new RegExp(forbidden.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), `release checklist missing ${forbidden}`)
  }
})
