...
function createMemoryOrders(initial = []) {
  ...
  return {
    ...
    async completePendingReviewOrder(id, data) {
      const order = records.find((item) => item._id === id && item.status === 'pending_review')
      if (!order) return null
      Object.assign(order, data)
      return { ...order }
    }
...
function createMemoryReviews(initial = []) {
  ...
  return {
    ...
    async create(data) {
      ...
      const record = { ...data, _id: data._id || `review_${records.length + 1}` }
      records.push(record)
      return { ...record }
    },
    async findById(id) {
...
function createMemoryCollection(prefix, initial = []) {
  ...
  return {
    records,
    async create(data) {
      const record = { ...data, _id: data._id || `${prefix}_${records.length + 1}` }
      records.push(record)
      return { ...record }
    },
    async findById(id) {
...
function createMemoryAppeals(initial = []) {
  ...
  return {
    ...
    async findPendingByReviewId(reviewId) {
      const appeal = base.records.find((item) => item.review_id === reviewId && item.status === 'pending')
      return appeal ? { ...appeal } : null
    },
    async findByReviewId(reviewId) {
      return base.records.filter((item) => item.review_id === reviewId).map((item) => ({ ...item }))
    },
    async findByWorkerId(workerId) {
      return base.records.filter((item) => item.worker_id === workerId).map((item) => ({ ...item }))
    }
  }
}
...
function createMemoryTipLogs(initial = []) {
  ...
  return {
    ...base,
    async findByOrderId(orderId) {
      return base.records.filter((item) => item.order_id === orderId).map((item) => ({ ...item }))
    },
...
function createReviewEnv(openid = 'openid_user') {
  return {
    ...
    finance: { async generateOrderFinance() { return { success: true } } },
  ...
  return {
    ...
    workerEarnings: {
      ...
      async findActiveByTipId(tipId) {
        const earning = this.records.find((item) => item.tip_id === tipId && item.status !== 'reversed')
        return earning ? { ...earning } : null
      },
      async findByWorkerId(workerId) {
        return this.records.filter((item) => item.worker_id === workerId).map((item) => ({ ...item }))
      }
    },
...
test('user creates enhanced review with tags, images, anonymity, bad level, message, and log', async () => {
  ...
  assert.equal(env.messages.records.some((item) => item.type === 'review_created'), true)
  assert.equal(env.reviewActionLogs.records.some((item) => item.action === 'create_review'), true)
...
test('user can add one followup to own completed review', async () => {
  ...
  assert.equal(result.success, true)
  assert.equal(result.data.review.followup_content, '后来又补做了一次，体验不错')
  assert.deepEqual(result.data.review.followup_images, ['cloud://followup.png'])
  assert.equal(env.messages.records.some((item) => item.type === 'review_followup_added'), true)
  assert.equal(env.reviewActionLogs.records.some((item) => item.action === 'add_followup'), true)
...
test('worker appeals bad review and admin approves or rejects appeal with logs', async () => {
  ...
  assert.equal(env.reviewActionLogs.records.some((item) => item.action === 'appeal_create'), true)
  ...
  assert.equal(env.reviewActionLogs.records.some((item) => item.action === 'appeal_approve'), true)
...
test('admin manages review visibility and normal user cannot hide reviews', async () => {
  ...
  assert.equal(env.reviewActionLogs.records.some((item) => item.action === 'hide_review'), true)
  ...
  assert.equal(env.reviewActionLogs.records.some((item) => item.action === 'restore_review'), true)
...
test('user creates one mock tip for completed own order and generates finance records', async () => {
  ...
  assert.equal(env.messages.records.some((item) => item.type === 'tip_created'), true)
...
test('tip permissions isolate worker list and allow admin all logs', async () => {
  const { handleTip } = require('../cloudfunctions/tip/handler')
  const env = createTipEnv()
  await handleTip({ action: 'createMockTip', orderId: 'order_completed', amount: 1000 }, env)
  ...
  const workerList = await handleTip({ action: 'getWorkerTipList' }, { ...env, openid: 'openid_worker' })
  assert.equal(workerList.success, true)
  assert.deepEqual(workerList.data.tips.map((item) => item.worker_id), ['openid_worker'])

  const adminList = await handleTip({ action: 'adminGetTipLogs' }, { ...env, openid: 'openid_admin' })
  assert.equal(adminList.success, true)
  assert.equal(adminList.data.tips.length, 2)

  const denied = await handleTip({ action: 'adminGetTipLogs' }, { ...env, openid: 'openid_user' })
  assert.equal(denied.success, false)
  assert.equal(denied.errorCode, 'PERMISSION_DENIED')
})
