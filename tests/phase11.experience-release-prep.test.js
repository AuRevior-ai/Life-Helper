...
function createMemoryOrders(initialOrders = []) {
  ...
  return {
    records,
    async findByUserId(userId) {
      return records.filter((order) => order.user_id === userId).map((order) => ({ ...order }))
    },
...
function createMemoryMessages(initialMessages = []) {
  ...
  return {
    ...
    async findByUserId(userId) {
      return records.filter((message) => message.user_id === userId).map((message) => ({ ...message }))
    },
    async findById(id) {
...
test('finishService saves required finish remark and up to three finish images', async () => {
  ...
  const env = {
    ...
    workers: createMemoryWorkers([{ user_id: 'openid_worker', audit_status: 'approved' }]),
    orders,
    messages: createMemoryMessages(),
    now: fixedNow
  }

  const missingRemark = await handleOrder({ action: 'finishService', orderId: 'order_1' }, env)
  ...
  assert.deepEqual(result.data.order.finish_images, ['cloud://finish-1', 'cloud://finish-2'])
})
...
test('order and review status changes create in-app messages without breaking flow', async () => {
  ...
  const orders = createMemoryOrders([
    { _id: 'order_1', user_id: 'openid_user', worker_id: '', status: 'pending_pay', pay_status: 'unpaid' },
    { _id: 'order_2', user_id: 'openid_user', worker_id: 'openid_worker', status: 'pending_review', pay_status: 'paid', service_id: 'svc_1', service_name: '日常保洁' }
  ])

  await handleOrder({ action: 'mockPayOrder', orderId: 'order_1' }, { openid: 'openid_user', orders, messages, now: fixedNow })
  await handleOrder({ action: 'acceptOrder', orderId: 'order_1' }, {
    openid: 'openid_worker',
    workers: createMemoryWorkers([{ user_id: 'openid_worker', audit_status: 'approved' }]),
    orders,
    messages,
  ...
  await handleReview({ action: 'createReview', orderId: 'order_2', rating: 5, content: '很好' }, {
    openid: 'openid_user',
    orders,
...
test('worker audit approval and rejection create in-app messages for worker', async () => {
  ...
  const users = {
    async findByOpenid(openid) {
      if (openid === 'openid_admin') return { _id: 'admin_1', openid, role: 'admin', status: 'normal' }
      if (openid === 'openid_worker') return { _id: 'user_worker', openid, role: 'user', status: 'normal' }
      return null
    },
    async updateById() {
      return {}
  ...
  await handleWorker({ action: 'approveWorker', workerId: 'worker_1' }, {
    openid: 'openid_admin',
    users,
    workers: createMemoryWorkers([{ _id: 'worker_1', user_id: 'openid_worker', audit_status: 'pending' }]),
    messages,
    now: fixedNow
  })
  await handleWorker({ action: 'rejectWorker', workerId: 'worker_2', reason: '资料不完整' }, {
    openid: 'openid_admin',
    users,
    workers: createMemoryWorkers([{ _id: 'worker_2', user_id: 'openid_worker', audit_status: 'pending' }]),
    messages,
    now: fixedNow
  })

  assert.deepEqual(messages.records.map((message) => message.type), ['worker_approved', 'worker_rejected'])
  assert.equal(messages.records.every((message) => message.user_id === 'openid_worker'), true)
})
...
test('message center lists only current user messages and marks messages read', async () => {
  ...
  const messages = createMemoryMessages([
    { _id: 'message_1', user_id: 'openid_user', title: '订单消息', is_read: false, created_at: new Date('2026-05-30T08:00:00.000Z') },
    { _id: 'message_2', user_id: 'openid_other', title: '其他消息', is_read: false, created_at: new Date('2026-05-30T08:00:00.000Z') }
  ])

  const listResult = await handleMessage({ action: 'getMessageList' }, { openid: 'openid_user', messages, now: fixedNow })
  assert.equal(listResult.success, true)
  assert.deepEqual(listResult.data.list.map((message) => message._id), ['message_1'])
  assert.equal(listResult.data.unread_count, 1)

  const readResult = await handleMessage({ action: 'markMessageRead', messageId: 'message_1' }, { openid: 'openid_user', messages, now: fixedNow })
...
test('worker detail accepts order worker_id and returns stats and reviews', async () => {
  const { handleWorker } = require('../cloudfunctions/worker/handler')
  const result = await handleWorker({ action: 'getWorkerDetail', workerId: 'openid_worker' }, {
    openid: 'openid_user',
    workers: createMemoryWorkers([{ _id: 'worker_1', user_id: 'openid_worker', name: '王师傅', audit_status: 'approved' }]),
    orders: createMemoryOrders([
    ...
    reviews: createMemoryReviews([
      { _id: 'review_1', worker_id: 'openid_worker', rating: 5, content: '很好' },
      { _id: 'review_2', worker_id: 'openid_worker', rating: 3, content: '准时' }
    ]),
...
test('order list actions paginate and preserve user worker admin permissions', async () => {
  ...
  const orders = createMemoryOrders([
    { _id: 'order_1', user_id: 'openid_user', worker_id: 'openid_worker', status: 'completed', category_id: 'cat_housekeeping', service_name: '日常保洁' },
    { _id: 'order_2', user_id: 'openid_user', worker_id: 'openid_worker', status: 'serving', category_id: 'cat_repair', service_name: '水电检修' },
    { _id: 'order_3', user_id: 'openid_other', worker_id: 'openid_other_worker', status: 'completed', category_id: 'cat_housekeeping', service_name: '玻璃清洁' }
  ])

  const userResult = await handleOrder({ action: 'getUserOrderList', status: 'completed', page: 1, pageSize: 1 }, { openid: 'openid_user', orders, now: fixedNow })
  assert.equal(userResult.success, true)
  assert.deepEqual(userResult.data.list.map((order) => order._id), ['order_1'])
  assert.equal(userResult.data.total, 1)
  assert.equal(userResult.data.hasMore, false)

  const workerResult = await handleOrder({ action: 'getWorkerOrderList', page: 1, pageSize: 5 }, {
    openid: 'openid_worker',
    workers: createMemoryWorkers([{ user_id: 'openid_worker', audit_status: 'approved' }]),
    orders,
    now: fixedNow
  })
  assert.deepEqual(workerResult.data.list.map((order) => order._id), ['order_1', 'order_2'])

  const adminResult = await handleAdmin({ action: 'getAllOrders', category_id: 'cat_housekeeping', page: 1, pageSize: 10 }, {
    openid: 'openid_admin',
    users: createMemoryUsers([{ _id: 'admin_1', openid: 'openid_admin', role: 'admin', status: 'normal' }]),
    orders,
    workers: { async findByAuditStatus() { return [] } },
    now: fixedNow
  })
  assert.deepEqual(adminResult.data.list.map((order) => order._id), ['order_1', 'order_3'])
})
...
test('phase eleven pages and services are wired', () => {
  ...
  const workerDetail = read('miniprogram/pages/worker/order-detail/order-detail.wxml')
  const userOrderDetail = read('miniprogram/pages/order-detail/order-detail.js')
