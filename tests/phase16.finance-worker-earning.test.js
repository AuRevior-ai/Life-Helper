  ...
  return {
    ...
    async findActiveByOrderId(orderId) {
      const record = records.find(
        (item) => item.order_id === orderId && item.status !== "reversed",
      );
...
test("completed paid order generates finance logs and one frozen worker earning", async () => {
  ...
  const result = await handleFinance(
    { action: "generateOrderFinance", orderId: "order_completed" },
    env,
  );
  ...
  assert.deepEqual(
    env.financeLogs.records.map((item) => item.type),
    ["order_income", "platform_commission", "worker_earning"],
  );
...
test("finance generation is idempotent and rejects invalid orders", async () => {
  ...
  const firstResult = await handleFinance(
    { action: "generateOrderFinance", orderId: "order_completed" },
    env,
  );
  const secondResult = await handleFinance(
    { action: "generateOrderFinance", orderId: "order_completed" },
    env,
  );
  ...
  const unpaidResult = await handleFinance(
    { action: "generateOrderFinance", orderId: "order_unpaid" },
    env,
  );
  assert.equal(unpaidResult.success, false);
  assert.equal(unpaidResult.errorCode, "ORDER_NOT_PAID");

  const pendingResult = await handleFinance(
    { action: "generateOrderFinance", orderId: "order_pending" },
    env,
  );
  assert.equal(pendingResult.success, false);
  assert.equal(pendingResult.errorCode, "ORDER_NOT_COMPLETED");

  const missingWorkerResult = await handleFinance(
    { action: "generateOrderFinance", orderId: "order_without_worker" },
    env,
  );
...
test("worker income summary and list are limited to current worker", async () => {
  ...
  const summaryResult = await handleFinance(
    { action: "getWorkerIncomeSummary" },
    env,
  );
  ...
  const listResult = await handleFinance(
...
test("admin can read finance logs, worker earnings, and order finance detail", async () => {
  ...
  const logsResult = await handleFinance(
  ...
  const earningsResult = await handleFinance(
  ...
  const userResult = await handleFinance(
    { action: "adminGetFinanceLogs" },
    {
      ...env,
      openid: "openid_user",
...
test("mock refund reverses unsettled earnings and settled earnings require manual handling", async () => {
  ...
  assert.equal(
    env.financeLogs.records.filter((item) => item.type === "earning_reverse")
      .length,
    1,
  );
...
test("review completion triggers retryable finance generation", async () => {
  ...
  orders.completePendingReviewOrder = async (id, data) =>
    orders.updateById(id, data);
...
test("mock refund triggers retryable finance reversal", async () => {
  ...
  const env = {
    ...
    refundLogs: {
      ...
      async create(data) {
        const record = {
          ...data,
          _id: `refund_log_${this.records.length + 1}`,
        };
  ...
  const result = await handleRefund(
    { action: "mockRefund", afterSaleId: "after_sale_1" },
    env,
  );
