...
test("getServiceList returns recommended services when no category is provided", async () => {
  ...
  assert.equal(
    result.data.services.every((service) => service.recommended),
    true,
  );
});
...
test("home and service pages call service APIs and expose navigation hooks", () => {
  const indexJs = fs.readFileSync(
    path.join(rootDir, "miniprogram/pages/index/index.js"),
    "utf8",
  );
  const listJs = fs.readFileSync(
    path.join(rootDir, "miniprogram/pages/service-list/service-list.js"),
    "utf8",
  );
  ...
  const indexWxml = fs.readFileSync(
    path.join(rootDir, "miniprogram/pages/index/index.wxml"),
    "utf8",
  );
  const listWxml = fs.readFileSync(
    path.join(rootDir, "miniprogram/pages/service-list/service-list.wxml"),
    "utf8",
  );
  const detailWxml = fs.readFileSync(
    path.join(rootDir, "miniprogram/pages/service-detail/service-detail.wxml"),
    "utf8",
  );
