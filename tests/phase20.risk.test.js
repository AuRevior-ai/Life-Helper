...
test('administrator can set risk level and tags while merchant sees simplified risk status only', async () => {
  ...
  const merchantView = await handleQualification({ action: 'getMyRiskStatus' }, env)
  ...
  assert.equal(Object.prototype.hasOwnProperty.call(merchantView.data, 'risk_tags'), false)
  assert.equal(Object.prototype.hasOwnProperty.call(merchantView.data, 'risk_reason'), false)
})
