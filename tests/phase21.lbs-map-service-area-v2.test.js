...
function createMemoryServiceProviders(initialProviders = []) {
  ...
  return {
    ...
    async findByRef(providerType, refId) {
      const provider = collection.records.find(
        (item) => item.provider_type === providerType && item.ref_id === refId,
      );
...
test("shared LBS utilities calculate distance and match radius, admin area, and legacy text", () => {
  ...
  assert.equal(
    isWithinRadius(
      ...
      { latitude: 30.2841, longitude: 120.1551 },
      2,
    ),
    true,
  );
  assert.equal(
    isWithinRadius(
      { latitude: null, longitude: 120.1551 },
      { latitude: 30.2841, longitude: 120.1551 },
      2,
    ),
    false,
  ...
  assert.equal(
    matchAdminArea(
      { adcode: "330106", community: "未来小区" },
      { service_adcodes: ["330106"], service_communities: ["其他小区"] },
    ).matched,
    true,
  );
  assert.equal(
    ...
    LBS_MATCH_RESULT.LEGACY_COMPAT,
  );
  assert.equal(
    matchAdminArea({ district: "西湖区" }, { service_districts: ["上城区"] })
      .matched,
    false,
  );
  ...
  const radius = matchProviderServiceRange(
    { latitude: 30.2741, longitude: 120.1551, community: "未来小区" },
    {
      ...
      base_latitude: 30.2742,
      base_longitude: 120.1552,
      service_radius_km: 1,
    },
  );
  ...
  const sorted = sortProvidersByDistance(
    { latitude: 30.2741, longitude: 120.1551 },
    [
...
test("address and order save latitude, longitude, POI, adcode, and address snapshot", async () => {
  ...
  assert.equal(
    orderResult.data.order.address_snapshot.map_poi_name,
    "未来小区东门",
  );
});
...
test("service area stores map center fields and exposes map list action", async () => {
  ...
  const mapList = await handleArea(
    { action: "adminGetServiceAreaMapList", includeDisabled: true },
    env,
  );
...
test("worker service range supports radius mode and order hall filters by LBS while preserving legacy rules", async () => {
  ...
  const result = await handleWorker(
    ...
    { openid: "openid_worker", workers, orders, now: fixedNow },
  );
  assert.equal(result.success, true);
  assert.deepEqual(
    result.data.orders.map((order) => order._id),
    ["near_order", "legacy_order"],
  );
  ...
  const paused = await handleWorker(
    { action: "getOrderHallList" },
    { openid: "openid_worker", workers, orders, now: fixedNow },
  );
...
test("admin assignment filters candidates with LBS metadata and can include service providers", async () => {
  ...
  const assignable = await handleDispatch(
    { action: "getAssignableWorkers", orderId: "order_1" },
    env,
  );
  assert.equal(assignable.success, true);
  assert.deepEqual(
    assignable.data.workers.map((worker) => worker._id),
    ["worker_1"],
  );
  assert.equal(assignable.data.workers[0].lbs_match.match_type, "radius");

  const providers = await handleDispatch(
    { action: "getAssignableProviders", orderId: "order_1" },
    env,
  );
  assert.equal(providers.success, true);
  assert.deepEqual(
    providers.data.providers.map((provider) => provider._id),
    ["worker_1", "provider_merchant"],
  );
...
test("merchant store list sorts by distance and keeps stores without location last", async () => {
  ...
  const env = {
    ...
    merchants: createMemoryMerchants([
      ...
      {
        _id: "merchant_missing",
        user_id: "openid_missing",
        store_name: "无坐标店",
        audit_status: "approved",
        status: "normal",
      },
      {
        ...
        audit_status: "approved",
        status: "normal",
        base_latitude: 30.275,
        base_longitude: 120.155,
      },
    ]),
  };
  const result = await handleMerchant(
    { action: "getStoreList", latitude: 30.2741, longitude: 120.1551 },
    env,
  );
  assert.equal(result.success, true);
  assert.deepEqual(
    result.data.list.map((merchant) => merchant._id),
    ["merchant_near", "merchant_far", "merchant_missing"],
  );
...
test("phase 21 routes, constants, services, docs, and real-device checklist are wired", () => {
  ...
  const docs =
    read("docs/map-lbs-setup.md") +
    read("docs/dev-records/21_lbs-map-service-area-v2.md") +
    read("README.md") +
    read("docs/release-package-checklist.md");
