...
function createMemoryWorkerEarnings(initialEarnings = []) {
  ...
  return {
    ...
    async findByOrderId(orderId) {
      return collection.records
        .filter((earning) => earning.order_id === orderId)
        .map((earning) => ({ ...earning }));
    },
    async findActiveByOrderId(orderId) {
      return (
        collection.records.find(
          (earning) =>
            earning.order_id === orderId && earning.status !== "reversed",
        ) || null
      );
    },
    async findByWorkerId(workerId) {
      return collection.records
        .filter((earning) => earning.worker_id === workerId)
        .map((earning) => ({ ...earning }));
    },
  };
}
...
test("merchant apply, admin audit, and service provider sync are isolated by permission", async () => {
  ...
  const duplicateResult = await handleMerchant(
    { action: "applyMerchant", storeName: "重复店铺" },
    env,
  );
  assert.equal(duplicateResult.success, false);
  assert.equal(duplicateResult.errorCode, "MERCHANT_ALREADY_APPLIED");

  const denyResult = await handleMerchant(
    {
...
test("merchant service configuration and public store pages expose only approved normal stores", async () => {
  ...
  const storeListResult = await handleMerchant(
    { action: "getStoreList" },
    { ...env, openid: "openid_user" },
  );
  assert.equal(storeListResult.success, true);
  assert.deepEqual(
    storeListResult.data.list.map((store) => store._id),
    ["merchant_1"],
  );
...
test("user creates merchant order and worker hall does not expose merchant orders", async () => {
  ...
  assert.equal(
    createResult.data.order.provider_snapshot.store_name,
    "未来家政店",
  );
  ...
  await handleOrder(
    { action: "mockPayOrder", orderId: createResult.data.order._id },
    {
      openid: "openid_user",
      orders,
      messages: createMemoryMessages(),
  ...
  const hallResult = await handleWorker(
  ...
  const acceptResult = await handleOrder(
    { action: "acceptOrder", orderId: createResult.data.order._id },
    {
      openid: "openid_worker",
      workers: createMemoryWorkers([
        {
...
test("merchant can operate own merchant order and finance records provider compatibility", async () => {
  ...
  const listResult = await handleMerchant(
    { action: "getMerchantOrderList" },
    env,
  );
  assert.equal(listResult.success, true);
  assert.deepEqual(
    listResult.data.list.map((order) => order._id),
    ["order_1"],
  );

  const otherResult = await handleMerchant(
    { action: "getMerchantOrderDetail", orderId: "order_1" },
    { ...env, openid: "openid_other" },
  );
  assert.equal(otherResult.success, false);
  assert.equal(otherResult.errorCode, "PERMISSION_DENIED");

  const acceptResult = await handleMerchant(
    { action: "merchantAcceptOrder", orderId: "order_1" },
    env,
  );
  assert.equal(acceptResult.success, true);
  assert.equal(acceptResult.data.order.status, "accepted");

  const startResult = await handleMerchant(
    { action: "merchantStartService", orderId: "order_1" },
    env,
  );
  ...
  assert.equal(
    env.merchantLogs.records.some((log) => log.action === "finish_service"),
    true,
  );
  ...
  const financeResult = await handleFinance(
    { action: "generateOrderFinance", orderId: "order_1" },
    {
      openid: "system",
      orders,
      financeLogs,
