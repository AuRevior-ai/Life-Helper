...
function createMemoryAfterSales(initialAfterSales = []) {
  ...
  return {
    records,
    async create(data) {
      const record = { ...data, _id: data._id || `after_sale_${records.length + 1}` }
      records.push(record)
      return { ...record }
    },
    async findById(id) {
    ...
    async findActiveByOrderId(orderId) {
      const record = records.find((item) =>
        item.order_id === orderId && ['pending', 'approved'].includes(item.status)
      )
      return record ? { ...record } : null
    },
    async findByUserId(userId) {
      return records.filter((record) => record.user_id === userId).map((record) => ({ ...record }))
    },
    async findAll() {
...
function createMemoryRefundLogs(initialLogs = []) {
  ...
  return {
    ...
    async findByAfterSaleId(afterSaleId) {
      return records.filter((record) => record.after_sale_id === afterSaleId).map((record) => ({ ...record }))
    }
  }
}
...
test('user can create after sale for own paid order with backend amount and message', async () => {
  ...
  assert.equal(env.messages.records.some((message) => message.type === 'after_sale_created'), true)
...
test('admin approval performs mock refund, logs it, updates order, and blocks duplicate review', async () => {
  ...
  assert.equal(env.refundLogs.records.some((log) => log.refund_status === 'mock_success'), true)
  assert.equal(env.messages.records.some((message) => message.type === 'refund_success'), true)
...
test('admin rejection does not create successful refund and direct duplicate refund is blocked', async () => {
  ...
  assert.equal(env.refundLogs.records.some((log) => log.refund_status === 'mock_success'), false)
  assert.equal(env.messages.records.some((message) => message.type === 'after_sale_rejected'), true)
  ...
  assert.equal(refundedEnv.refundLogs.records.some((log) => log.type === 'duplicate_refund'), true)
})
...
test('after sale pages, constants, collections, and services are wired', () => {
  ...
  assert.equal(exists('miniprogram/pages/admin/after-sale-detail/after-sale-detail.js'), true)
