...
test('review appeal actions hide loading before showing explicit success feedback', () => {
  const workerReview = read('miniprogram/pages/worker/review-detail/review-detail.js')
  const adminAppeal = read('miniprogram/pages/admin/review-appeal-detail/review-appeal-detail.js')
  const adminReview = read('miniprogram/pages/admin/review-detail/review-detail.js')

  assert.match(workerReview, /hideLoading\(\)[\s\S]*showSuccess\('申诉提交成功'/)
  assert.match(workerReview, /hideLoading\(\)[\s\S]*showSuccess\('回复提交成功'/)
  assert.match(adminAppeal, /hideLoading\(\)[\s\S]*showSuccess\('申诉审核已通过'|hideLoading\(\)[\s\S]*showSuccess\('申诉审核已拒绝'/)
...
test('merchant and admin merchant operations show clear success and failure feedback', () => {
  const adminMerchant = read('miniprogram/pages/admin/merchant-detail/merchant-detail.js')
  const merchantOrder = read('miniprogram/pages/merchant/order-detail/order-detail.js')
  const merchantApply = read('miniprogram/pages/merchant/apply/apply.js')
  const merchantServiceEdit = read('miniprogram/pages/merchant/service-edit/service-edit.js')
  ...
  assert.match(merchantServiceEdit, /showError\(error\.message \|\| '保存失败'\)/)
})
