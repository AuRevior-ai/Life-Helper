...
test("admin manages service areas and users only see enabled areas", async () => {
  ...
  assert.equal(
    createResult.data.area.full_name,
    "杭州 西湖区 古荡街道 未来小区",
  );
  ...
  const userListResult = await handleArea(
    { action: "getServiceAreaList" },
    {
      openid: "openid_user",
      users: createMemoryUsers([
  ...
  const adminListResult = await handleArea(
...
test("address and order save structured service area snapshots", async () => {
  ...
  assert.equal(
    addressResult.data.address.full_address,
    "杭州 西湖区 古荡街道 未来小区 1 幢 101",
  );
  ...
  assert.equal(
    orderResult.data.order.full_address,
    "杭州 西湖区 古荡街道 未来小区 1 幢 101",
  );
});
...
test("worker area config and online status control order hall visibility", async () => {
  ...
  const visibleResult = await handleWorker(
    { action: "getOrderHallList" },
    baseEnv,
  );
  assert.equal(visibleResult.success, true);
  assert.deepEqual(
    visibleResult.data.orders.map((item) => item._id),
    ["order_a"],
  );
  ...
  const pausedList = await handleWorker(
    { action: "getOrderHallList" },
    baseEnv,
  );
  ...
  assert.deepEqual(areaResult.data.worker.service_communities, [
    "未来小区",
    "星河小区",
  ]);
});
...
test("admin assigns and unassigns only matching available workers with logs and messages", async () => {
  ...
  const assignableResult = await handleDispatch(
    { action: "getAssignableWorkers", orderId: "order_1" },
    env,
  );
  assert.deepEqual(
    assignableResult.data.workers.map((item) => item._id),
    ["worker_match"],
  );
  ...
  const userResult = await handleDispatch(
    { action: "getAssignableWorkers", orderId: "order_1" },
    {
      ...env,
      openid: "openid_user",
...
test("phase 15 routes, services, constants, and docs are wired", () => {
  ...
  const adminOrderDetailJs = read(
    "miniprogram/pages/admin/order-detail/order-detail.js",
  );
