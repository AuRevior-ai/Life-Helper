...
test('worker order detail loads and renders user review followup content', () => {
  const workerOrderJs = read('miniprogram/pages/worker/order-detail/order-detail.js')
  const workerOrderWxml = read('miniprogram/pages/worker/order-detail/order-detail.wxml')
...
test('admin area and dispatch lists tolerate missing real-env collections', async () => {
  ...
  const areaResult = await handleArea({ action: 'getServiceAreaList', includeDisabled: true }, {
    openid: 'openid_admin',
  ...
  const dispatchResult = await handleDispatch({ action: 'getDispatchLogs' }, {
    openid: 'openid_admin',
    users: createAdminUsers(),
...
test('admin review and appeal lists group records by worker', () => {
  const reviewListJs = read('miniprogram/pages/admin/review-list/review-list.js')
  const reviewListWxml = read('miniprogram/pages/admin/review-list/review-list.wxml')
  const appealListJs = read('miniprogram/pages/admin/review-appeal-list/review-appeal-list.js')
  const appealListWxml = read('miniprogram/pages/admin/review-appeal-list/review-appeal-list.wxml')
...
test('admin appeal review returns to previous page after successful processing', () => {
  const appealDetailJs = read('miniprogram/pages/admin/review-appeal-detail/review-appeal-detail.js')
...
test('merchant audit status page uses action-service data and shows next actions', () => {
  const auditStatusJs = read('miniprogram/pages/merchant/audit-status/audit-status.js')
  const auditStatusWxml = read('miniprogram/pages/merchant/audit-status/audit-status.wxml')
