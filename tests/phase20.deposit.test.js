...
test("mock deposit payment creates mock-only record and never returns real pay params", async () => {
  ...
  assert.equal(
    Object.prototype.hasOwnProperty.call(paid.data, "payParams"),
    false,
  );
  assert.equal(
    Object.prototype.hasOwnProperty.call(paid.data, "prepay_id"),
    false,
  );
  assert.equal(
...
test("deposit refund and freeze actions require administrator review", async () => {
  ...
  const refund = await handleQualification(
    { action: "applyDepositRefund", reason: "关闭店铺" },
    env,
  );
  assert.equal(refund.success, true);
  assert.equal(refund.data.deposit.deposit_status, "REFUND_PENDING");

  const freezeDenied = await handleQualification(
    {
      action: "adminFreezeDeposit",
  ...
  const refundAgain = await handleQualification(
    { action: "applyDepositRefund", reason: "再次申请" },
    env,
  );
