...
function createAdminEnv() {
  return {
    ...
    orders: createMemoryOrders([
      { _id: 'order_1', status: 'completed', price: 9900, service_name: '日常保洁' },
      { _id: 'order_2', status: 'pending_review', price: 6900, service_name: '水电检修' }
    ]),
...
test('admin getDashboard returns MVP management stats', async () => {
  ...
  const result = await handleAdmin({ action: 'getDashboard' }, createAdminEnv())
...
test('admin can list and disable users', async () => {
  ...
  assert.equal(env.users.records.find((user) => user._id === 'user_1').status, 'disabled')
})
...
test('admin can list orders and manually update order status', async () => {
  ...
  assert.equal(env.orders.records.find((order) => order._id === 'order_2').updated_at.toISOString(), '2026-05-30T15:30:00.000Z')
})
...
test('service seed migration upserts seed categories and services into repositories', async () => {
  ...
  assert.equal(env.categories.records.some((category) => category._id === 'cat_housekeeping'), true)
  assert.equal(env.services.records.some((service) => service._id === 'svc_home_daily_clean'), true)
})
...
test('admin pages are wired to dashboard, orders, users, and service management APIs', () => {
  ...
  const categoryListJs = fs.readFileSync(
    path.join(rootDir, 'miniprogram/pages/admin/category-list/category-list.js'),
    'utf8'
  )
  ...
  const serviceListWxml = fs.readFileSync(
    path.join(rootDir, 'miniprogram/pages/admin/service-list/service-list.wxml'),
    'utf8'
  )
