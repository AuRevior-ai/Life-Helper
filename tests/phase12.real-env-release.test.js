...
function createMemoryOrders(initialOrders = []) {
  ...
  return {
    ...
    async findByServiceId(serviceId) {
      return records
        .filter((order) => order.service_id === serviceId)
        .map((order) => ({ ...order }));
    },
  };
}
...
test("adminGetWorkerDetail requires admin and returns worker stats", async () => {
  ...
  const adminResult = await handleWorker(
    { action: "adminGetWorkerDetail", workerId: "worker_1" },
    env,
  );
  ...
  const userResult = await handleWorker(
    { action: "adminGetWorkerDetail", workerId: "worker_1" },
    env,
  );
...
test("service category and service deletion are protected by related records", async () => {
  ...
  const env = {
    ...
    services: createMemoryCollection([
      {
        _id: "svc_used",
        category_id: "cat_used",
        name: "已有订单服务",
        status: "off",
      },
    ]),
    orders: createMemoryOrders([{ _id: "order_1", service_id: "svc_used" }]),
    now: fixedNow,
  };

  const categoryResult = await handleService(
    { action: "deleteCategory", categoryId: "cat_used" },
    env,
  );
  assert.equal(categoryResult.success, false);
  assert.equal(categoryResult.errorCode, "CATEGORY_HAS_SERVICES");

  const serviceResult = await handleService(
    { action: "deleteService", serviceId: "svc_used" },
    env,
  );
  assert.equal(serviceResult.success, false);
  assert.equal(serviceResult.errorCode, "SERVICE_HAS_ORDERS");

  const emptyCategoryResult = await handleService(
    { action: "deleteCategory", categoryId: "cat_empty" },
    env,
  );
