...
test("onboarding status depends on qualification, deposit, risk, and manual limit", async () => {
  ...
  const incomplete = await handleQualification(
  ...
  const waiting = await handleQualification(
  ...
  const depositWait = await handleQualification(
    { action: "getOnboardingStatus" },
    env,
  );
  ...
  const active = await handleQualification(
    { action: "getOnboardingStatus" },
    env,
  );
  ...
  const blocked = await handleQualification(
    { action: "getOnboardingStatus" },
    env,
  );
...
test("merchant service publishing is blocked when onboarding risk is blocked", async () => {
  ...
  const qualifications = createOwnerRepository(
    createMemoryCollection([
      {
        _id: "q1",
  ...
  const deposits = createOwnerRepository(
    createMemoryCollection([
      {
        _id: "d1",
        merchant_id: "merchant_1",
  ...
  const riskRecords = createMemoryCollection([
    {
      ...
      provider_type: "merchant",
      risk_level: "BLOCKED",
      risk_tags: ["MANUAL_REVIEW_REQUIRED"],
      created_at: fixedNow(),
    },
  ]);
  const env = {
    ...
    users: createMemoryUsers([
      { _id: "u1", openid: "openid_merchant", role: "user", status: "normal" },
    ]),
  ...
  const result = await handleMerchant(
    { action: "createMerchantService", serviceId: "svc_clean", price: 8800 },
    env,
  );
