...
test("startService moves assigned accepted order to serving", async () => {
  ...
  assert.equal(
    result.data.order.started_at.toISOString(),
...
test("finishService moves assigned serving order to pending review", async () => {
  ...
  assert.equal(
    result.data.order.finished_at.toISOString(),
    "2026-05-30T12:30:00.000Z",
  );
});
...
test("createReview creates review and completes user pending-review order", async () => {
  ...
  assert.equal(
    orders.records.find((order) => order._id === "order_review").status,
    "completed",
  );
  assert.equal(
    orders.records
      .find((order) => order._id === "order_review")
