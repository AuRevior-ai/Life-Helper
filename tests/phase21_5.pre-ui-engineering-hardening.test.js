...
test('key pages remain registered in app.json', () => {
  ...
  for (const page of requiredPages) {
    assert.equal(appJson.pages.includes(page), true, `${page} should stay registered`)
  }
})
...
test('shared sync check script is wired into package scripts and passes on current tree', () => {
  ...
  assert.equal(pkg.scripts['check:shared-sync'], 'node scripts/check-cloudfunction-shared-sync.js')
