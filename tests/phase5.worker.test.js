...
test('applyWorker creates a pending worker profile for current user', async () => {
  ...
  assert.equal(result.data.worker.created_at.toISOString(), '2026-05-30T09:30:00.000Z')
})
...
test('admin approves a pending worker and promotes user role', async () => {
  ...
  assert.equal(users.records.find((user) => user._id === 'user_worker').role, 'worker')
})
...
test('approved worker accepts pending order and owns worker order list and detail', async () => {
  ...
  assert.equal(acceptResult.data.order.accepted_at.toISOString(), '2026-05-30T09:30:00.000Z')
