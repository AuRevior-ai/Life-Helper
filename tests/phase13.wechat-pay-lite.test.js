...
test("payment cloud function creates prepay only for payable own orders", async () => {
  ...
  const payableResult = await handlePayment(
    { action: "createPayment", orderId: "order_payable" },
    env,
  );
  assert.equal(payableResult.success, true);
  assert.equal(
    payableResult.data.payParams.package,
    "prepay_id=prepay_order_payable",
  );
  ...
  assert.equal(
    paymentLogs.records.some((log) => log.type === "create_prepay"),
    true,
  );

  const paidResult = await handlePayment(
    { action: "createPayment", orderId: "order_paid" },
    env,
  );
  assert.equal(paidResult.success, false);
  assert.equal(paidResult.errorCode, "ORDER_ALREADY_PAID");

  const canceledResult = await handlePayment(
    { action: "createPayment", orderId: "order_canceled" },
    env,
  );
...
test("payment status query is limited to current user order ownership", async () => {
  ...
  assert.equal(
    paymentLogs.records.some((log) => log.type === "query_payment"),
    true,
  );
...
test("payment pages and services are wired while mock payment remains available", () => {
  ...
  assert.match(
    payResultJs,
    /order\.pay_amount\s*\|\|\s*order\.payable_amount\s*\|\|\s*order\.price/,
  );
});
