...
function createOwnerRepository(collection, ownerKeys = ['merchant_id', 'provider_id']) {
  return {
    ...collection,
    async findByOwner(query = {}) {
      const record = collection.records.find((item) =>
    ...
    async findByStatus(status) {
      return collection.records.filter((item) => item.qualification_status === status || item.deposit_status === status).map((item) => ({ ...item }))
    },
...
function createQualificationEnv(overrides = {}) {
  ...
  return {
    ...
    users: createMemoryUsers([
      ...
      { _id: 'user_other', openid: 'openid_other', role: 'user', status: 'normal' },
    ...
    merchants: createMemoryMerchants([
      ...
      { _id: 'merchant_2', user_id: 'openid_other', store_name: '其他店', audit_status: 'approved', status: 'normal' }
    ]),
    qualifications,
    deposits,
    riskRecords,
