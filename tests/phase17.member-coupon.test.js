...
function createMemoryMemberships(initial = []) {
  ...
  return {
    ...
    async findActiveByUserId(userId, now) {
      const record = base.records.find(
        (item) =>
          item.user_id === userId &&
          item.status === "active" &&
  ...
  return {
    ...
    async findByUserAndTemplate(userId, templateId) {
      return base.records
        .filter(
          (item) =>
            item.user_id === userId && item.coupon_template_id === templateId,
        )
        .map((item) => ({ ...item }));
    },
...
test("user can view member plans and mock open membership", async () => {
  ...
  assert.deepEqual(
    plansResult.data.plans.map((item) => item.level),
    ["monthly", "seasonly", "yearly"],
  );

  const openResult = await handlePromotion(
    { action: "mockOpenMembership", level: "monthly" },
    env,
  );
...
test("active higher member level cannot be overwritten by lower level plan", async () => {
  ...
  const yearlyResult = await handlePromotion(
    { action: "mockOpenMembership", level: "yearly" },
    env,
  );
  assert.equal(yearlyResult.success, true);
  assert.equal(yearlyResult.data.membership.level, "yearly");

  const seasonlyResult = await handlePromotion(
    { action: "mockOpenMembership", level: "seasonly" },
    env,
  );
...
test("order creation saves promotion snapshots, payment uses coupon, cancel releases coupon, and finance uses payable amount", async () => {
  ...
  assert.equal(
    createResult.data.order.coupon_snapshot.user_coupon_id,
    "coupon_1",
  );
...
test("phase 17 frontend, constants, cloud function, and docs are wired", () => {
  ...
  const orderSubmitWxml = read(
    "miniprogram/pages/order-submit/order-submit.wxml",
  );
