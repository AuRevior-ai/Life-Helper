...
test("qualification draft, submit, review, and sensitive masking follow mock state machine", async () => {
  ...
  const initial = await handleQualification(
    { action: "getMyQualification" },
    env,
  );
  ...
  const submit = await handleQualification(
    { action: "submitQualification" },
    env,
  );
  assert.equal(submit.success, true);
  assert.equal(
    submit.data.qualification.qualification_status,
    "PENDING_REVIEW",
  );
  ...
  assert.equal(
    env.onboardingLogs.records.at(-1).event_type,
...
test("rejected or supplement-required qualification can be resubmitted by owner only", async () => {
  ...
  const submitted = await handleQualification(
    { action: "submitQualification" },
    env,
  );
  ...
  const otherResubmit = await handleQualification(
    { action: "resubmitQualification" },
  ...
  const ownerResubmit = await handleQualification(
    { action: "resubmitQualification" },
    env,
  );
  assert.equal(ownerResubmit.success, true);
  assert.equal(
    ownerResubmit.data.qualification._id,
    draft.data.qualification._id,
  );
  assert.equal(
    ownerResubmit.data.qualification.qualification_status,
    "PENDING_REVIEW",
  );
