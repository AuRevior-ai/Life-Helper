...
test('phase 20 actions, pages, schemas, and documentation are synchronized', () => {
  ...
  const record = read('docs/dev-records/20_deposit-qualification-certification.md')
  ...
  for (const action of actions) {
    assert.ok(manifest.qualification.actions.includes(action), `${action} missing from manifest`)
  ...
  for (const snippet of [
    ...
    assert.match(statusDoc + dbDoc + permissionDoc + paginationDoc + readme + record, new RegExp(snippet.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
  }
