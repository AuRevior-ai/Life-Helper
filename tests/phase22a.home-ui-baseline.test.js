...
test("user tab pages remove the inherited dark native navigation bar", () => {
  ...
  for (const page of tabPages) {
    ...
    assert.equal(
      pageJson.navigationStyle,
      "custom",
      `${page.jsonPath} should use custom navigation`,
    );
    assert.equal(
      pageJson.navigationBarTitleText,
      undefined,
      `${page.jsonPath} should not render native title text`,
    );
...
test("home page defines the three common service rows", () => {
  ...
  for (const text of [
    ...
    assert.match(
      `${js}\n${wxml}`,
      new RegExp(text),
      `common service baseline should include ${text}`,
    );
  }
});
...
test("home service categories use managed local image assets", () => {
  ...
  assert.match(
    wxml,
    /<image[^>]+class="home-category-image"[^>]+src="{{ item\.iconPath }}"/,
  );
...
test("home image assets stay small enough for real-device debugging package limits", () => {
  ...
  for (const file of assetFiles) {
    ...
    totalSize += size;
    assert.ok(
      size <= maxSingleAssetSize,
      `${file} is ${size} bytes and should stay under ${maxSingleAssetSize}`,
    );
  }
...
test("home common service rows use managed local cover images", () => {
  ...
  assert.match(
    wxml,
    /<image[^>]+class="home-service-cover-image"[^>]+src="{{ item\.coverPath }}"/,
  );
  assert.match(wxml, /mode="aspectFill"/);
  ...
  assert.doesNotMatch(
    wxss,
    /\.home-service-cover--cleaning\s*{[^}]*linear-gradient/s,
  );
  assert.doesNotMatch(
    wxss,
    /\.home-service-cover--repair\s*{[^}]*linear-gradient/s,
  );
  assert.doesNotMatch(
    wxss,
    /\.home-service-cover--pet\s*{[^}]*linear-gradient/s,
  );
});
