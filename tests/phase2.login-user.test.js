...
test('loginOrRegister creates a default normal user on first login', async () => {
  ...
  assert.equal(result.data.user.created_at.toISOString(), '2026-05-30T03:30:00.000Z')
  assert.equal(result.data.user.updated_at.toISOString(), '2026-05-30T03:30:00.000Z')
...
test('normal user cannot update another user role', async () => {
  ...
  assert.equal(users.records.find((user) => user._id === 'user_2').role, 'user')
})
