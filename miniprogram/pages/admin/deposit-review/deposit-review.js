...
function isCollectionMissing(error = {}) {
  return /DATABASE_COLLECTION_NOT_EXIST|collection not exists|Db or Table not exist|merchant_deposits/.test(error.message || '')
}
...
Page({
  ...
  async loadList() {
    ...
    try {
      ...
      this.setData({ deposits: data.deposits || data.list || [], collectionMissing: data.collection_missing === true })
    } catch (error) {
  ...
  async freeze(event) {
    try {
      await qualificationService.adminFreezeDeposit({ depositId: event.currentTarget.dataset.id, reason: '阶段 20 mock 风险复核' })
      showSuccess('保证金已冻结')
      await this.loadList()
    } catch (error) {
