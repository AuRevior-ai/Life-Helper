...
Page({
  ...
  async loadFormData() {
    try {
      ...
      this.setData({
        ...
        selectedCategoryName: categories[0] ? categories[0].name : '请先创建分类'
      })
  ...
  applyService(service) {
    const categoryIndex = Math.max(
      this.data.categories.findIndex((category) => category._id === service.category_id),
      0
    )
    const statusIndex = service.status === 'off' ? 1 : 0
    this.setData({
      categoryIndex,
      selectedCategoryName: this.data.categoryNames[categoryIndex] || '请先创建分类',
      statusIndex,
