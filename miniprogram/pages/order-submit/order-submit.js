...
Page({
  ...
  async loadPageData() {
    ...
    try {
      ...
      this.setData({
        service: null,
        loadError: error.message || "下单信息加载失败",
      });
  ...
  async loadMerchantServiceSnapshot() {
    const data = await merchantService.getStoreServices({
      merchantServiceId: this.data.merchantServiceId,
    });
    const service =
      (data.services || []).find(
        (item) => item._id === this.data.merchantServiceId,
      ) || data.service;
    return {
      service: {
  ...
  findSelectedAddress(addresses) {
    ...
    const previous = addresses.find(
      (address) => address._id === this.data.selectedAddressId,
    );
  ...
  selectAddress(event) {
    ...
    const selectedAddress = this.data.addresses.find(
  ...
  async loadPromotionPreview() {
    ...
    try {
      ...
      const couponLabels = ["不使用优惠券"].concat(
        availableCoupons.map((coupon) => coupon.coupon_name || coupon.name),
      );
  ...
  async refreshPromotionAmount() {
    ...
    try {
      ...
      this.setData({
        originalAmountText: formatPrice(
          data.original_amount || this.data.service.price,
        ),
        ...
        payableAmountText: formatPrice(
          data.payable_amount || this.data.service.price,
        ),
      });
    } catch (error) {
      this.setData({
