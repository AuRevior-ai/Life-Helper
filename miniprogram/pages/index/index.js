...
Page({
  ...
  async loadHomeData() {
    ...
    try {
      ...
      this.setData({
        ...
        displayCategories: buildDisplayCategories(categoryData.categories || []),
  ...
  goServiceList(event) {
    const categoryId = event && event.currentTarget && event.currentTarget.dataset.id
    wx.navigateTo({
      url: categoryId ? `/pages/service-list/service-list?categoryId=${categoryId}` : '/pages/service-list/service-list'
    })
  },
