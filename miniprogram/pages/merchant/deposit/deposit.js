...
const { DEPOSIT_STATUS, DEPOSIT_STATUS_TEXT } = require('../../../config/status')
...
function buildDepositView(deposit = {}) {
  ...
  const canMockPay = [DEPOSIT_STATUS.UNPAID, DEPOSIT_STATUS.MOCK_PAYING].includes(status)
  const canApplyRefund = [DEPOSIT_STATUS.MOCK_PAID, DEPOSIT_STATUS.FROZEN, DEPOSIT_STATUS.REFUND_REJECTED].includes(status)
  return {
    status,
    ...
    helperText: status === DEPOSIT_STATUS.MOCK_PAID
      ? '已模拟缴纳，保证金流程已通过。'
...
Page({
  ...
  async applyRefund() {
    try {
      await qualificationService.applyDepositRefund({ reason: '商家申请模拟退还' })
      showSuccess('退还申请已提交')
