...
const ACTIONS_BY_STATUS = {
  ...
  accepted: [
    { type: 'detail', label: '联系师傅', style: 'primary' }
  ],
  serving: [
    { type: 'detail', label: '联系师傅', style: 'primary' }
  ],
  pending_review: [
    { type: 'review', label: '去评价', style: 'primary' }
  ],
  completed: [
    { type: 'detail', label: '查看详情', style: 'secondary' }
  ]
}
...
Component({
  ...
  observers: {
    'order, order.price, order.pay_amount, order.payable_amount, order.status, order.full_address, order.address, order.community': function updateDisplayText(order, price, payAmount, payableAmount, status) {
