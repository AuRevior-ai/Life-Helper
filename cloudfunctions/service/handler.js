...
async function getCategoryList(event = {}, env = {}) {
  ...
  if (!payload.includeDisabled) {
    categories = categories.filter(
      (category) => category.status === CATEGORY_STATUS.ENABLED,
    );
  }
...
async function getServiceList(event = {}, env = {}) {
  ...
  if (!payload.includeOff) {
    services = services.filter(
      (service) => service.status === SERVICE_STATUS.ON,
    );
  }

  if (payload.categoryId) {
    services = services.filter(
      (service) => service.category_id === payload.categoryId,
    );
  }
...
async function getServiceDetail(event = {}, env = {}) {
  ...
  if (
    !service ||
    (!payload.includeOff && service.status !== SERVICE_STATUS.ON)
  ) {
...
async function seedServiceData(event = {}, env = {}) {
  ...
  await Promise.all(
    SERVICE_CATEGORIES.map((category) =>
      env.categories.upsert({ ...category }),
    ),
  );
  await Promise.all(
    SERVICES.map((service) => env.services.upsert({ ...service })),
  );
...
async function updateCategory(event = {}, env = {}) {
  ...
  ["name", "icon", "description", "status", "sort"].forEach((field) => {
    if (payload[field] !== undefined) {
      data[field] =
        field === "sort" ? Number(payload[field] || 0) : payload[field];
    }
  });
...
async function deleteCategory(event = {}, env = {}) {
  ...
  const relatedServices = await getServicesByCategoryId(
    payload.categoryId,
    env,
  );
...
async function updateService(event = {}, env = {}) {
  ...
  if (payload.categoryName !== undefined)
    data.category_name = payload.categoryName;
