...
function calculateFinance(order = {}, config = {}) {
  ...
  const commissionRateBps = Number(
    config.commissionRateBps || DEFAULT_PLATFORM_COMMISSION_RATE_BPS,
  );
  const platformCommissionAmount = Math.round(
    (paidAmount * commissionRateBps) / 10000,
  );
  const workerEarningAmount = Math.max(
    paidAmount - platformCommissionAmount,
    0,
  );
  return {
...
async function generateOrderFinance(event = {}, env = {}) {
  ...
  const providerType =
    order.provider_type || (order.merchant_id ? "merchant" : "worker");
  const providerId =
    order.provider_id || order.worker_id || order.merchant_id || "";
...
async function reverseOrderFinance(event = {}, env = {}) {
  ...
  const currentEarning =
    allEarnings.find(
      (item) => item.status !== WORKER_EARNING_STATUS.REVERSED,
    ) || allEarnings[0];
  ...
  const refundAmount = Number(
    payload.refundAmount ||
      order.refund_amount ||
      currentEarning.paid_amount ||
      0,
  );
  const financeNo = createFinanceNo(env);

  if (
    currentEarning.status === WORKER_EARNING_STATUS.SETTLED ||
    currentEarning.settlement_status === SETTLEMENT_STATUS.SETTLED
  ) {
    const workerEarning = await env.workerEarnings.updateById(
      currentEarning._id,
      {
    ...
    await createFinanceLog(env, {
      ...
      platform_commission_amount:
        currentEarning.platform_commission_amount || 0,
  ...
  const baseLog = {
    ...
    paid_amount: currentEarning.paid_amount || getPaidAmount(order),
    commission_rate:
      currentEarning.commission_rate ||
      DEFAULT_PLATFORM_COMMISSION_RATE_BPS / 10000,
    commission_rate_bps:
      currentEarning.commission_rate_bps ||
      DEFAULT_PLATFORM_COMMISSION_RATE_BPS,
...
async function adminGetOrderFinanceDetail(event = {}, env = {}) {
  ...
  const logs = env.financeLogs.findByOrderId
    ? await env.financeLogs.findByOrderId(order._id)
    : [];
  const earnings = env.workerEarnings.findByOrderId
    ? await env.workerEarnings.findByOrderId(order._id)
    : [];
...
async function mockUnlockSettlement(event = {}, env = {}) {
  ...
  for (const earning of allEarnings) {
    const frozenUntil = earning.frozen_until
      ? new Date(earning.frozen_until)
      : null;
    if (
      earning.status === WORKER_EARNING_STATUS.FROZEN &&
      (!frozenUntil || frozenUntil <= now)
    ) {
