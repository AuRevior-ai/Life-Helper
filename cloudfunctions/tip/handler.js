...
async function requireTippableOrder(orderId, env) {
  ...
  if (order.user_id !== requireOpenid(env)) throw serviceError('PERMISSION_DENIED', '无权打赏该订单')
  if (order.status !== ORDER_STATUS.COMPLETED || order.pay_status !== PAY_STATUS.PAID) {
    throw serviceError('ORDER_STATUS_INVALID', '只有已完成且已支付订单可以打赏')
  }
  if (!order.worker_id) throw serviceError('ORDER_WORKER_MISSING', '订单没有师傅，不能打赏')
...
async function createMockTip(event = {}, env = {}) {
  ...
  const existingTips = env.tipLogs.findByOrderId ? await env.tipLogs.findByOrderId(order._id) : []
  if (existingTips.some((tip) => tip.status !== TIP_STATUS.REVERSED)) {
    throw serviceError('TIP_ALREADY_EXISTS', '该订单已打赏')
  }

  const now = getNow(env)
  const amounts = calculateTipAmount(normalizeAmount(payload.amount), DEFAULT_TIP_COMMISSION_RATE)
...
async function handleTip(event = {}, env = {}) {
  ...
  try {
    ...
    return fail(error.errorCode || 'INTERNAL_ERROR', error.message || '打赏操作失败')
  }
}
