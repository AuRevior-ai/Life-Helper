...
function isWithinRadius(pointA = {}, pointB = {}, radiusKm) {
  ...
  const distance = calculateDistanceKm(pointA.latitude, pointA.longitude, pointB.latitude, pointB.longitude)
...
function matchAdminArea(orderAddress = {}, provider = {}) {
  const orderAdcode = trimText(orderAddress.adcode || orderAddress.district_code)
  const providerAdcodes = normalizeList(provider.service_adcodes || provider.adcodes || provider.adcode)
  if (orderAdcode && providerAdcodes.length) {
    return providerAdcodes.includes(orderAdcode)
      ? { matched: true, match_type: 'admin_area', distance_km: null, reason: LBS_MATCH_RESULT.MATCHED_BY_ADMIN_AREA }
      : { matched: false, match_type: 'none', distance_km: null, reason: LBS_MATCH_RESULT.NOT_MATCHED }
  }

  if (listMatches(orderAddress.community, provider.service_communities || provider.communities || provider.service_area)) {
    return { matched: true, match_type: 'legacy_text', distance_km: null, reason: LBS_MATCH_RESULT.LEGACY_COMPAT }
  }
  if (listMatches(orderAddress.street, provider.service_streets || provider.streets)) {
    return { matched: true, match_type: 'admin_area', distance_km: null, reason: LBS_MATCH_RESULT.MATCHED_BY_ADMIN_AREA }
  }
  if (listMatches(orderAddress.district, provider.service_districts || provider.districts)) {
    return { matched: true, match_type: 'admin_area', distance_km: null, reason: LBS_MATCH_RESULT.MATCHED_BY_ADMIN_AREA }
  }
  ...
  return { matched: false, match_type: 'none', distance_km: null, reason: LBS_MATCH_RESULT.NOT_MATCHED }
}

function getProviderPoint(provider = {}) {
  return {
    latitude: provider.base_latitude ?? provider.latitude ?? provider.center_latitude,
    longitude: provider.base_longitude ?? provider.longitude ?? provider.center_longitude
  }
}
...
function matchProviderServiceRange(orderAddress = {}, provider = {}) {
  ...
  if (mode === SERVICE_RANGE_MODE.RADIUS) {
    ...
    if (hasPoint(orderAddress) && hasPoint(providerPoint)) {
      const distance = calculateDistanceKm(orderAddress.latitude, orderAddress.longitude, providerPoint.latitude, providerPoint.longitude)
      const matched = distance !== null && distance <= Number(provider.service_radius_km || 0)
      return {
        matched,
        match_type: matched ? 'radius' : 'none',
        distance_km: distance,
        reason: matched ? LBS_MATCH_RESULT.MATCHED_BY_RADIUS : LBS_MATCH_RESULT.NOT_MATCHED
      }
    }
    const fallback = matchAdminArea(orderAddress, provider)
    if (fallback.matched) return fallback
    return { matched: false, match_type: 'none', distance_km: null, reason: LBS_MATCH_RESULT.LOCATION_MISSING }
  }
...
function sortProvidersByDistance(point = {}, providers = []) {
  return [...providers]
    .map((provider) => {
      const providerPoint = getProviderPoint(provider)
      const distance = hasPoint(point) && hasPoint(providerPoint)
        ? calculateDistanceKm(point.latitude, point.longitude, providerPoint.latitude, providerPoint.longitude)
        : null
