...
async function createReview(event, env) {
  ...
  const review = await env.reviews.create({
    ...
    tags: normalizeStringArray(payload.tags, 10, 'REVIEW_TAGS_INVALID', '评价标签格式不正确'),
    content: trimText(payload.content),
    images: normalizeStringArray(payload.images, 3, 'REVIEW_IMAGES_INVALID', '评价图片最多 3 张'),
...
async function addReviewFollowup(event, env) {
  ...
  const openid = requireOpenid(env)
  if (review.user_id !== openid) throw serviceError('PERMISSION_DENIED', '无权追评该评价')
  ...
  const updated = await env.reviews.updateById(review._id, {
    ...
    followup_images: normalizeStringArray(payload.images || payload.followupImages, 3, 'REVIEW_IMAGES_INVALID', '追评图片最多 3 张'),
    followup_at: now,
    updated_at: now
  })
...
async function workerReplyReview(event, env) {
  ...
  const review = await requireReviewById(payload.reviewId, env)
  const openid = requireOpenid(env)
  if (review.worker_id !== openid) throw serviceError('PERMISSION_DENIED', '无权回复该评价')
...
async function workerCreateReviewAppeal(event, env) {
  ...
  const review = await requireReviewById(payload.reviewId, env)
  const openid = requireOpenid(env)
  if (review.worker_id !== openid) throw serviceError('PERMISSION_DENIED', '无权申诉该评价')
  if ((review.status || REVIEW_STATUS.VISIBLE) !== REVIEW_STATUS.VISIBLE || review.status === REVIEW_STATUS.DELETED) {
    throw serviceError('REVIEW_STATUS_INVALID', '当前评价不可申诉')
  }
  const isBad = review.rating_level === REVIEW_RATING_LEVEL.BAD || Number(review.rating || 0) <= 2
  if (!isBad) throw serviceError('REVIEW_APPEAL_NOT_ALLOWED', '只有差评可以申诉')
  const reason = trimText(payload.reason)
  if (!reason) throw serviceError('REVIEW_APPEAL_REASON_MISSING', '请填写申诉理由')
  const existing = env.reviewAppeals && env.reviewAppeals.findPendingByReviewId
    ? await env.reviewAppeals.findPendingByReviewId(review._id)
    : null
  if (existing) throw serviceError('REVIEW_APPEAL_PENDING', '该评价已有待处理申诉')
  ...
  const appeal = await env.reviewAppeals.create({
    ...
    images: normalizeStringArray(payload.images, 3, 'REVIEW_APPEAL_IMAGES_INVALID', '申诉图片最多 3 张'),
...
async function adminHideReview(event, env) {
  ...
  const payload = getPayload(event)
  const reason = trimText(payload.reason)
  if (!reason) throw serviceError('REVIEW_HIDE_REASON_MISSING', '请填写隐藏原因')
  const review = await requireReviewById(payload.reviewId, env)
  const now = getNow(env)
  const updated = await env.reviews.updateById(review._id, {
...
async function adminRestoreReview(event, env) {
  ...
  const payload = getPayload(event)
  const reason = trimText(payload.reason)
  if (!reason) throw serviceError('REVIEW_RESTORE_REASON_MISSING', '请填写恢复原因')
  const review = await requireReviewById(payload.reviewId, env)
  const now = getNow(env)
  const updated = await env.reviews.updateById(review._id, {
    status: REVIEW_STATUS.VISIBLE,
...
async function adminReviewAppeal(event, env) {
  ...
  if (!adminRemark) throw serviceError('REVIEW_APPEAL_REMARK_MISSING', '请填写审核备注')
  const approved = payload.result === REVIEW_APPEAL_STATUS.APPROVED || payload.status === REVIEW_APPEAL_STATUS.APPROVED
  const rejected = payload.result === REVIEW_APPEAL_STATUS.REJECTED || payload.status === REVIEW_APPEAL_STATUS.REJECTED
  if (!approved && !rejected) throw serviceError('REVIEW_APPEAL_RESULT_INVALID', '申诉处理结果不合法')

  const now = getNow(env)
  const nextStatus = approved ? REVIEW_APPEAL_STATUS.APPROVED : REVIEW_APPEAL_STATUS.REJECTED
  ...
  await safeCreateReviewActionLog(env, {
    ...
    order_id: review.order_id,
    order_no: review.order_no || '',
    action: approved ? REVIEW_ACTION_TYPE.APPEAL_APPROVE : REVIEW_ACTION_TYPE.APPEAL_REJECT,
    operator_id: admin.openid || requireOpenid(env),
    operator_role: 'admin',
...
async function getReviewDetail(event, env) {
  ...
  const appeals = env.reviewAppeals && env.reviewAppeals.findByReviewId
...
async function adminGetReviewList(event, env) {
  ...
  if (payload.status) reviews = reviews.filter((review) => (review.status || REVIEW_STATUS.VISIBLE) === payload.status)
  if (payload.ratingLevel) reviews = reviews.filter((review) => review.rating_level === payload.ratingLevel)
  if (payload.badOnly) reviews = reviews.filter((review) => review.rating_level === REVIEW_RATING_LEVEL.BAD || Number(review.rating || 0) <= 2)
  return success({ reviews })
}
...
async function adminGetReviewAppealList(event, env) {
  ...
  if (payload.status) appeals = appeals.filter((appeal) => appeal.status === payload.status)
