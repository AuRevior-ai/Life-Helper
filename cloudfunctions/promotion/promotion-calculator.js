...
function calculateMemberDiscount(originalAmount, membership) {
  ...
  if (!Number.isFinite(discountRate) || discountRate <= 0 || discountRate >= 1) return 0
  return Math.min(Math.round(originalAmount * (1 - discountRate)), originalAmount)
}

function calculateCouponDiscount(amountAfterMember, coupon = {}, originalAmount = amountAfterMember) {
  ...
  if (coupon.type === COUPON_TYPE.DISCOUNT) {
    ...
    return Math.min(maxDiscount ? Math.min(discount, maxDiscount) : discount, amountAfterMember)
  }
...
function calculateOrderPromotion({ service, membership, coupon }) {
  ...
  const memberDiscountAmount = calculateMemberDiscount(originalAmount, membership)
  const amountAfterMember = Math.max(originalAmount - memberDiscountAmount, 0)
  const couponDiscountAmount = calculateCouponDiscount(amountAfterMember, coupon, originalAmount)
  const totalDiscountAmount = Math.min(memberDiscountAmount + couponDiscountAmount, originalAmount)
  ...
  return {
    ...
    promotion_source: buildPromotionSource(memberDiscountAmount, couponDiscountAmount),
