...
async function listMemberPlans(env = {}) {
  const plans = env.memberPlans && env.memberPlans.findAll
...
async function mockOpenMembership(event = {}, env = {}) {
  ...
  if (!plan) throw serviceError('MEMBER_PLAN_NOT_FOUND', '会员方案不存在或已停用')

  const now = getNow(env)
  const existing = env.memberships.findByUserId
    ? await env.memberships.findByUserId(userId)
    : null
  const hasActiveExisting = existing && existing.status === MEMBER_STATUS.ACTIVE && new Date(existing.expired_at) > now
  if (hasActiveExisting && getMemberLevelRank(existing.level) > getMemberLevelRank(plan.level)) {
    throw serviceError('MEMBER_PLAN_DOWNGRADE_NOT_ALLOWED', '当前已有更高等级会员，不能开通较低等级会员')
  }
  const startedAt = existing && existing.status === MEMBER_STATUS.ACTIVE && new Date(existing.expired_at) > now
    ? new Date(existing.started_at || now)
    : now
  const baseExpire = existing && existing.status === MEMBER_STATUS.ACTIVE && new Date(existing.expired_at) > now
  ...
  const membership = existing && env.memberships.updateById
...
async function getMyMembership(event = {}, env = {}) {
  ...
  const membership = env.memberships.findByUserId
    ? await env.memberships.findByUserId(userId)
    : null
  if (membership && membership.status === MEMBER_STATUS.ACTIVE && new Date(membership.expired_at) <= now) {
...
function normalizeTemplatePayload(payload = {}, now) {
  return {
    ...
    applicable_category_ids: Array.isArray(payload.applicable_category_ids) ? payload.applicable_category_ids : [],
...
async function adminUpdateCouponTemplate(event = {}, env = {}) {
  await requireAdmin(env)
  const payload = getPayload(event)
  if (!payload.couponTemplateId) throw serviceError('COUPON_TEMPLATE_ID_MISSING', '缺少优惠券模板 ID')
  const template = await env.couponTemplates.updateById(
    payload.couponTemplateId,
    normalizeTemplatePayload(payload, getNow(env))
  )
  if (!template) throw serviceError('COUPON_TEMPLATE_NOT_FOUND', '优惠券模板不存在')
  return success({ template })
}
...
async function adminEnableCouponTemplate(event = {}, env = {}) {
  await requireAdmin(env)
  const payload = getPayload(event)
  const template = await env.couponTemplates.updateById(payload.couponTemplateId, {
...
async function getReceivableCoupons(event = {}, env = {}) {
  const templates = env.couponTemplates.findActive
    ...
    : (await env.couponTemplates.findAll()).filter((item) => item.status === COUPON_STATUS.ACTIVE)
  return success({ templates })
}

function isTemplateReceivable(template = {}, now) {
  if (template.status !== COUPON_STATUS.ACTIVE) return false
  if (template.total_quantity && Number(template.received_quantity || 0) >= Number(template.total_quantity)) return false
  if (template.valid_end_at && new Date(template.valid_end_at) <= now) return false
  return true
}
...
async function receiveCoupon(event = {}, env = {}) {
  ...
  const validStartAt = now > new Date(template.valid_start_at || now) ? now : new Date(template.valid_start_at || now)
  const userCoupon = await env.userCoupons.create({
    ...
    valid_end_at: addDays(validStartAt, Number(template.valid_days_after_receive || 7)),
    used_at: null,
    used_order_id: '',
    locked_order_id: '',
    created_at: now,
...
function isCouponValidForOrder(coupon = {}, service = {}, now) {
  ...
  if (coupon.valid_start_at && new Date(coupon.valid_start_at) > now) return false
  if (coupon.valid_end_at && new Date(coupon.valid_end_at) <= now) return false
  const categories = coupon.applicable_category_ids || []
  if (categories.length && !categories.includes(service.category_id)) return false
  return true
}
...
async function resolveUserCoupon(userId, couponId, service, env) {
  ...
  if (!coupon || coupon.user_id !== userId || !isCouponValidForOrder(coupon, service, getNow(env))) {
...
async function calculateOrderPromotion(event = {}, env = {}) {
  ...
  const coupon = await resolveUserCoupon(userId, payload.userCouponId, service, env)
...
async function getAvailableCouponsForOrder(event = {}, env = {}) {
  ...
  const availableCoupons = coupons.filter((coupon) => {
    ...
    return calculatePromotion({ service, membership, coupon }).coupon_discount_amount > 0
  })
...
async function lockCouponForOrder(event = {}, env = {}) {
  ...
  if (coupon && coupon.status === USER_COUPON_STATUS.LOCKED && coupon.locked_order_id === payload.orderId) {
    return success({ already_locked: true, userCoupon: coupon })
  }
  const userCoupon = env.userCoupons.lockUnusedCoupon
    ? await env.userCoupons.lockUnusedCoupon(payload.userCouponId, userId, payload.orderId, { updated_at: getNow(env) })
...
async function useCouponForOrder(event = {}, env = {}) {
  ...
  if (!payload.userCouponId) return success({ userCoupon: null })
  const coupon = await env.userCoupons.findById(payload.userCouponId)
  if (coupon && coupon.status === USER_COUPON_STATUS.USED && coupon.used_order_id === payload.orderId) {
    return success({ already_used: true, userCoupon: coupon })
  }
  const userCoupon = env.userCoupons.useLockedCoupon
    ? await env.userCoupons.useLockedCoupon(payload.userCouponId, payload.orderId, {
      used_at: getNow(env),
      updated_at: getNow(env)
    })
    : await env.userCoupons.updateById(payload.userCouponId, {
...
async function releaseCouponForOrder(event = {}, env = {}) {
  ...
  if (!payload.userCouponId) return success({ userCoupon: null })
  const coupon = await env.userCoupons.findById(payload.userCouponId)
  if (coupon && coupon.status === USER_COUPON_STATUS.UNUSED && !coupon.locked_order_id) {
    return success({ already_released: true, userCoupon: coupon })
  }
  const userCoupon = env.userCoupons.releaseLockedCoupon
    ? await env.userCoupons.releaseLockedCoupon(payload.userCouponId, payload.orderId, { updated_at: getNow(env) })
    : await env.userCoupons.updateById(payload.userCouponId, {
      status: USER_COUPON_STATUS.UNUSED,
      locked_order_id: '',
      updated_at: getNow(env)
    })
  if (!userCoupon) throw serviceError('COUPON_RELEASE_FAILED', '优惠券释放失败')
  return success({ userCoupon })
}
...
async function handlePromotion(event = {}, env = {}) {
  ...
  try {
    ...
    return fail(error.errorCode || 'INTERNAL_ERROR', error.message || '营销操作失败')
  }
}
