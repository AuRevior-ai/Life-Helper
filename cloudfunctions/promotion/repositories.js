...
function createCouponTemplateRepository(db) {
  ...
  return {
    ...
    async findActive() {
      const result = await couponTemplates
        .where({ status: "active" })
...
function createUserCouponRepository(db) {
  ...
  return {
    ...
    async findByUserId(userId) {
      const result = await userCoupons
        .where({ user_id: userId })
        .orderBy("created_at", "desc")
        .get();
      return result.data || [];
    ...
    async findByUserAndTemplate(userId, templateId) {
      const result = await userCoupons
        .where({ user_id: userId, coupon_template_id: templateId })
        .get();
      return result.data || [];
    },
    async updateById(id, data) {
    ...
    async lockUnusedCoupon(id, userId, orderId, data) {
      const result = await userCoupons
        .where({ _id: id, user_id: userId, status: "unused" })
        .update({
    ...
    async useLockedCoupon(id, orderId, data) {
      const result = await userCoupons
        .where({ _id: id, status: "locked", locked_order_id: orderId })
        .update({
