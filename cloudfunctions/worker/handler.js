...
function orderMatchesWorkerCategory(order, worker) {
  ...
  return hasTextMatch(
    workerCategory,
    order.category_name || order.category_id || order.service_category,
  );
}
...
function normalizeWorkerPayload(payload = {}) {
  const serviceAreaIds = Array.isArray(
    payload.service_area_ids || payload.serviceAreaIds,
  )
    ? (payload.service_area_ids || payload.serviceAreaIds)
        .map((item) => trimText(item))
        .filter(Boolean)
    : [];
  const serviceCommunities = Array.isArray(
    payload.service_communities || payload.serviceCommunities,
  )
    ? (payload.service_communities || payload.serviceCommunities)
        .map((item) => trimText(item))
        .filter(Boolean)
  ...
  return {
    ...
    service_category: trimText(
      payload.service_category || payload.serviceCategory,
    ),
    ...
    service_districts: Array.isArray(
      payload.service_districts || payload.serviceDistricts,
    )
      ? (payload.service_districts || payload.serviceDistricts)
          .map((item) => trimText(item))
          .filter(Boolean)
      : [],
    service_streets: Array.isArray(
      payload.service_streets || payload.serviceStreets,
    )
      ? (payload.service_streets || payload.serviceStreets)
          .map((item) => trimText(item))
          .filter(Boolean)
      : [],
    service_adcodes: Array.isArray(
      payload.service_adcodes || payload.serviceAdcodes,
    )
      ? (payload.service_adcodes || payload.serviceAdcodes)
          .map((item) => trimText(item))
          .filter(Boolean)
      : [],
    service_range_mode:
      trimText(payload.service_range_mode || payload.serviceRangeMode) ||
      "admin_area",
    base_latitude: toNumberOrNull(
      payload.base_latitude || payload.baseLatitude,
    ),
    base_longitude: toNumberOrNull(
      payload.base_longitude || payload.baseLongitude,
    ),
    ...
    service_radius_km: Number(
      payload.service_radius_km || payload.serviceRadiusKm || 0,
    ),
...
async function enrichWorkerAreas(payload, env) {
  ...
  const communities = areas
    .map((area) => trimText(area.community))
    .filter(Boolean);
  const districts = Array.from(
    new Set(areas.map((area) => trimText(area.district)).filter(Boolean)),
  );
  return {
    ...payload,
...
async function applyWorker(event, env) {
  ...
  const payload = await enrichWorkerAreas(
    normalizeWorkerPayload(getPayload(event)),
    env,
  );
  ...
  if (
    existingWorker &&
    existingWorker.audit_status === WORKER_AUDIT_STATUS.APPROVED
  ) {
  ...
  if (existingWorker) {
    const worker = await env.workers.updateById(existingWorker._id, {
      ...
      online_status:
        existingWorker.online_status || WORKER_ONLINE_STATUS.AVAILABLE,
      reject_reason: "",
      updated_at: now,
    });
...
async function updateWorkerServiceRange(event, env) {
  ...
  const serviceRange = {
    ...
    service_city: trimText(
      payload.service_city || payload.serviceCity || worker.service_city,
    ),
    service_districts: Array.isArray(
      payload.service_districts || payload.serviceDistricts,
    )
    ...
    service_communities: Array.isArray(
      payload.service_communities || payload.serviceCommunities,
    )
      ? (payload.service_communities || payload.serviceCommunities)
          .map((item) => trimText(item))
...
async function getWorkerDetail(event, env) {
  ...
  const orders =
    env.orders && env.orders.findByWorkerId
      ? await env.orders.findByWorkerId(workerUserId)
      : [];
  const reviews =
    env.reviews && env.reviews.findByWorkerId
