...
function normalizeAppointment(payload, env) {
  const appointmentDate = trimText(
    payload.appointment_date || payload.appointmentDate,
  );
  const appointmentSlot = trimText(
    payload.appointment_slot || payload.appointmentSlot,
  );
  const legacyTime = trimText(
  ...
  if (
    !isDateOnly(appointmentDate) ||
    !APPOINTMENT_TIME_SLOTS.includes(appointmentSlot)
  ) {
...
async function callPromotion(env, action, payload) {
  ...
  if (result && result.success === false) {
    throw serviceError(
      result.errorCode || "PROMOTION_ERROR",
      result.message || "营销优惠处理失败",
    );
  }
...
async function calculatePromotionSnapshot(env, service, payload) {
  ...
  try {
    const promotionResult = await callPromotion(
      env,
...
async function safeUseCouponForOrder(env, order) {
  const userCouponId =
    order && order.coupon_snapshot && order.coupon_snapshot.user_coupon_id;
...
async function findMerchantServiceSnapshot(merchantServiceId, env) {
  ...
  if (!env.merchantServices || !env.merchantServices.findById) {
    throw serviceError(
      "MERCHANT_SERVICE_REPOSITORY_MISSING",
      "缺少商家服务集合",
    );
  }
  ...
  if (
    ...
    env.serviceProviders.findById &&
    merchantService.provider_id
  ) {
...
async function createOrder(event, env) {
  ...
  const merchantServiceId = trimText(
    payload.merchantServiceId || payload.merchant_service_id,
  );
  const serviceId = merchantServiceId
    ? ""
    : requireText(payload.serviceId, "SERVICE_ID_MISSING", "缺少服务 ID");
  const addressId = requireText(
    payload.addressId,
    "ADDRESS_ID_MISSING",
    "缺少地址 ID",
  );
  const appointment = normalizeAppointment(payload, env);
  const merchantSnapshot = await findMerchantServiceSnapshot(
    merchantServiceId,
    env,
  );
  const service = merchantSnapshot
    ? merchantSnapshot.service
    : await findServiceById(serviceId, env);
  if (!service) {
  ...
  const promotionSnapshot = await calculatePromotionSnapshot(
    env,
    service,
    payload,
  );
  ...
  const providerId =
    merchantSnapshot && merchantSnapshot.provider
      ? merchantSnapshot.provider._id
      : "";
  const merchant = merchantSnapshot ? merchantSnapshot.merchant : null;
  const merchantService = merchantSnapshot
    ? merchantSnapshot.merchantService
    : null;
  ...
  const userCouponId =
    promotionSnapshot.coupon_snapshot &&
    promotionSnapshot.coupon_snapshot.user_coupon_id;
  if (userCouponId) {
    try {
...
async function mockPayOrder(event, env) {
  ...
  if (
    ...
    order.pay_status !== PAY_STATUS.UNPAID
  ) {
...
async function finishService(event, env) {
  ...
  const finishImages = normalizeFinishImages(
    payload.finish_images || payload.finishImages,
  );
  const now = getNow(env);
  const updatedOrder = await env.orders.updateById(order._id, {
...
async function cancelOrder(event, env) {
  ...
  if (
    ![ORDER_STATUS.PENDING_PAY, ORDER_STATUS.PENDING_ACCEPT].includes(
      order.status,
    )
  ) {
