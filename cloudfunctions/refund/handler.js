...
function isAfterSaleAllowed(order = {}) {
  return order.pay_status === 'paid' && ALLOWED_AFTER_SALE_ORDER_STATUS.includes(order.status)
}
...
async function getAfterSaleDetail(event = {}, env = {}) {
  ...
  const user = env.users && env.users.findByOpenid ? await env.users.findByOpenid(openid) : null
  if (afterSale.user_id !== openid && afterSale.worker_id !== openid && (!user || user.role !== USER_ROLE.ADMIN)) {
    throw serviceError('PERMISSION_DENIED', '无权查看该售后申请')
  }

  const order = await env.orders.findById(afterSale.order_id)
  const refundLogs = env.refundLogs && env.refundLogs.findByAfterSaleId
...
async function adminGetAfterSaleList(event = {}, env = {}) {
  ...
  if (payload.status) {
    afterSales = afterSales.filter((afterSale) => afterSale.status === payload.status)
  }
  return success({ afterSales })
}
...
async function performMockRefund(afterSale, env = {}) {
  ...
  if ([REFUND_STATUS.MOCK_SUCCESS, REFUND_STATUS.SUCCESS].includes(order.refund_status)) {
    await createRefundLog(env, {
      ...buildRefundLogData(afterSale, order, afterSale.refund_no || createRefundNo(env), {
...
async function mockRefund(event = {}, env = {}) {
  ...
  if (![AFTER_SALE_STATUS.APPROVED, AFTER_SALE_STATUS.REFUNDED].includes(afterSale.status)) {
...
async function getRefundLogs(event = {}, env = {}) {
  ...
  const openid = requireOpenid(env)
  const user = env.users && env.users.findByOpenid ? await env.users.findByOpenid(openid) : null
  if (afterSale.user_id !== openid && (!user || user.role !== USER_ROLE.ADMIN)) {
...
async function handleRefund(event = {}, env = {}) {
  ...
  try {
    ...
    return fail(error.errorCode || 'INTERNAL_ERROR', error.message || '售后退款操作失败')
  }
}
