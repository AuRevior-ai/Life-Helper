...
function normalizeArray(value) {
  return Array.isArray(value) ? value.map((item) => trimText(item)).filter(Boolean) : []
}
...
async function requireAdmin(env = {}) {
  ...
  if (user.role !== USER_ROLE.ADMIN) throw serviceError('PERMISSION_DENIED', '当前操作需要管理员权限')
  return user
}
...
function normalizeMerchantPayload(payload = {}) {
  return {
    ...
    business_license_image: trimText(payload.businessLicenseImage || payload.business_license_image),
    qualification_images: normalizeArray(payload.qualificationImages || payload.qualification_images),
    service_category_ids: normalizeArray(payload.serviceCategoryIds || payload.service_category_ids),
    service_area_ids: normalizeArray(payload.serviceAreaIds || payload.service_area_ids),
    service_communities: normalizeArray(payload.serviceCommunities || payload.service_communities),
...
async function applyMerchant(event, env) {
  ...
  if (existing) throw serviceError('MERCHANT_ALREADY_APPLIED', '一个账号只能申请一个商家')
...
async function upsertMerchantProvider(merchant, env) {
  ...
  if (env.serviceProviders.upsertByRef) {
    return env.serviceProviders.upsertByRef(SERVICE_PROVIDER_TYPE.MERCHANT, merchant._id, data)
  }
  const existing = await env.serviceProviders.findByRef(SERVICE_PROVIDER_TYPE.MERCHANT, merchant._id)
...
async function createMerchantService(event, env) {
  ...
  const provider = env.serviceProviders.findByRef
    ? await env.serviceProviders.findByRef(SERVICE_PROVIDER_TYPE.MERCHANT, merchant._id)
    : null
  const now = getNow(env)
  const merchantService = await env.merchantServices.create({
    ...
    cover_image: trimText(payload.coverImage || payload.cover_image) || service.cover_image || '',
...
async function requireOwnedMerchantService(merchantServiceId, merchant, env) {
  ...
  if (!merchantService) throw serviceError('MERCHANT_SERVICE_NOT_FOUND', '商家服务不存在')
  if (merchantService.merchant_id !== merchant._id) throw serviceError('PERMISSION_DENIED', '无权操作该服务')
...
async function updateMerchantServiceStatus(event, env, status) {
  const merchant = await requireOwnedApprovedMerchant(env)
  const payload = getPayload(event)
  const merchantService = await requireOwnedMerchantService(payload.merchantServiceId || payload.merchant_service_id, merchant, env)
...
function ensurePublicMerchant(merchant) {
  if (!merchant || merchant.audit_status !== MERCHANT_AUDIT_STATUS.APPROVED || merchant.status !== MERCHANT_STATUS.NORMAL) {
...
async function getStoreDetail(event, env) {
  ...
  const merchantServiceId = payload.merchantServiceId || payload.merchant_service_id
  if (!merchantId && merchantServiceId) {
    selectedService = await env.merchantServices.findById(merchantServiceId)
    if (!selectedService) throw serviceError('MERCHANT_SERVICE_NOT_FOUND', '商家服务不存在')
...
async function getStoreServices(event, env) {
  ...
  return success({ services: result.data.services, list: result.data.services })
}
...
async function requireOwnedMerchantOrder(orderId, merchant, env) {
  ...
  if (order.provider_type !== SERVICE_PROVIDER_TYPE.MERCHANT || order.merchant_id !== merchant._id) {
...
async function getMerchantOrderList(event, env) {
  ...
  let orders = env.orders.findByMerchantId
    ...
    : (await env.orders.findAll()).filter((order) => order.merchant_id === merchant._id)
  const payload = getPayload(event)
  if (payload.status) orders = orders.filter((order) => order.status === payload.status)
  return success({ list: orders, orders })
}

async function getMerchantOrderDetail(event, env) {
  const merchant = await requireOwnedApprovedMerchant(env)
  const order = await requireOwnedMerchantOrder(getPayload(event).orderId, merchant, env)
...
async function merchantAcceptOrder(event, env) {
  ...
  if (merchant.status !== MERCHANT_STATUS.NORMAL) throw serviceError('MERCHANT_DISABLED', '商家已被禁用')
  const order = await requireOwnedMerchantOrder(getPayload(event).orderId, merchant, env)
  if (order.status !== ORDER_STATUS.PENDING_ACCEPT) throw serviceError('ORDER_STATUS_INVALID', '当前订单不能接单')
  const now = getNow(env)
...
async function merchantStartService(event, env) {
  const merchant = await requireOwnedApprovedMerchant(env)
  const order = await requireOwnedMerchantOrder(getPayload(event).orderId, merchant, env)
  if (order.status !== ORDER_STATUS.ACCEPTED) throw serviceError('ORDER_STATUS_INVALID', '当前订单不能开始服务')
  const now = getNow(env)
  const updated = await env.orders.updateById(order._id, {
...
function normalizeFinishImages(value) {
  if (!value) return []
  if (!Array.isArray(value)) throw serviceError('FINISH_IMAGES_INVALID', '完工图片格式不正确')
  const images = value.map((item) => trimText(item)).filter(Boolean)
  if (images.length > 3) throw serviceError('FINISH_IMAGES_INVALID', '完工图片最多 3 张')
  return images
}
...
async function merchantFinishService(event, env) {
  ...
  if (order.status !== ORDER_STATUS.SERVING) throw serviceError('ORDER_STATUS_INVALID', '当前订单不能完成服务')
  const finishRemark = requireText(payload.finishRemark || payload.finish_remark, 'FINISH_REMARK_MISSING', '请填写完工说明')
  const finishImages = normalizeFinishImages(payload.finishImages || payload.finish_images)
  const now = getNow(env)
  const updated = await env.orders.updateById(order._id, {
...
async function adminGetMerchantDetail(event, env) {
  await requireAdmin(env)
  const merchant = await requireMerchantById(getPayload(event).merchantId, env)
  const services = env.merchantServices.findByMerchantId ? await env.merchantServices.findByMerchantId(merchant._id) : []
...
async function adminGetMerchantOrders(event, env) {
  await requireAdmin(env)
  const payload = getPayload(event)
  const orders = env.orders.findByMerchantId ? await env.orders.findByMerchantId(payload.merchantId) : []
  return success({ list: orders, orders })
}
...
async function handleMerchant(event = {}, env = {}) {
  ...
  try {
    ...
    return fail(error.errorCode || 'INTERNAL_ERROR', error.message || '商家操作失败')
  }
}
