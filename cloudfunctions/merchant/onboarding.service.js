...
async function findOwnedQualification(context, env) {
  ...
  if (env.qualifications.findByOwner) return env.qualifications.findByOwner(context)
  const list = env.qualifications.findAll ? await env.qualifications.findAll() : []
  return list.find((item) => item.merchant_id === context.merchant_id && item.provider_type === context.provider_type) || null
}
...
async function findLatestRisk(context, env) {
  ...
  const matched = list
    .filter((item) => item.merchant_id === context.merchant_id && item.provider_type === context.provider_type)
  return matched.reduce((latest, item) => {
    if (!latest) return item
    return new Date(item.created_at || 0) >= new Date(latest.created_at || 0) ? item : latest
  }, null)
}

function computeOnboardingStatus({ qualification, deposit, risk }) {
  if (risk && risk.risk_level === RISK_LEVEL.BLOCKED) return ONBOARDING_STATUS.BLOCKED
  if (risk && risk.risk_level === RISK_LEVEL.HIGH) return ONBOARDING_STATUS.RISK_REVIEW
  if (!qualification || qualification.qualification_status === QUALIFICATION_STATUS.NOT_SUBMITTED) return ONBOARDING_STATUS.INCOMPLETE
  if (qualification.manual_limited) return ONBOARDING_STATUS.LIMITED
  if (qualification.qualification_status === QUALIFICATION_STATUS.PENDING_REVIEW) return ONBOARDING_STATUS.QUALIFICATION_WAIT
  if ([QUALIFICATION_STATUS.REJECTED, QUALIFICATION_STATUS.NEED_SUPPLEMENT, QUALIFICATION_STATUS.DRAFT].includes(qualification.qualification_status)) {
    return ONBOARDING_STATUS.INCOMPLETE
  }
  if (qualification.qualification_status !== QUALIFICATION_STATUS.APPROVED) return ONBOARDING_STATUS.INCOMPLETE

  const depositStatus = deposit ? deposit.deposit_status : DEPOSIT_STATUS.UNPAID
  if (![DEPOSIT_STATUS.NOT_REQUIRED, DEPOSIT_STATUS.MOCK_PAID].includes(depositStatus)) return ONBOARDING_STATUS.DEPOSIT_WAIT
...
function getSafeOnboardingMessage(status) {
  ...
  if (status === ONBOARDING_STATUS.RISK_REVIEW) return '平台要求补充材料或等待复核'
  if (status === ONBOARDING_STATUS.DEPOSIT_WAIT) return '请先完成模拟保证金流程'
  if (status === ONBOARDING_STATUS.QUALIFICATION_WAIT) return '资质正在等待平台审核'
  return '请先完成模拟资质认证流程'
}
...
async function getOnboardingSnapshot(context, env) {
  ...
  const onboarding_status = computeOnboardingStatus({ qualification, deposit, risk })
  return {
    qualification,
    deposit,
    risk,
...
async function canMerchantOperate(context, env) {
  const snapshot = await getOnboardingSnapshot({
    ...
    provider_type: context.providerType || context.provider_type || PROVIDER_TYPE.MERCHANT
  }, env)
  return {
...
function assertCanMerchantOperate(result) {
  if (!result.can_operate) {
    throw serviceError('MERCHANT_ONBOARDING_BLOCKED', result.message || '商家入驻状态暂不可经营')
  }
}
