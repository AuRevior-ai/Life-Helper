...
function validateAdminBootstrapGuard(payload = {}, env = {}) {
  ...
  const allowedOpenids = normalizeList(
    env.config && env.config.adminBootstrapAllowedOpenids,
  );
  if (allowedOpenids.length > 0 && !allowedOpenids.includes(env.openid)) {
    throw serviceError(
      "ADMIN_BOOTSTRAP_OPENID_DENIED",
      "当前账号不在管理员初始化白名单",
    );
  }

  const expectedCode =
    `${(env.config && env.config.adminBootstrapCode) || ""}`.trim();
  if (
    expectedCode &&
    `${payload.initCode || payload.adminBootstrapCode || ""}`.trim() !==
      expectedCode
  ) {
