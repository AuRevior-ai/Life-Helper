...
async function requireCurrentUser(env = {}) {
  ...
  if (!user || user.status === 'disabled') throw serviceError('USER_NOT_FOUND', '用户不存在或已禁用')
  return user
}

async function requireAdmin(env = {}) {
  const user = await requireCurrentUser(env)
  if (user.role !== 'admin') throw serviceError('PERMISSION_DENIED', '当前操作需要管理员权限')
  return user
}

async function resolveMyProviderContext(env = {}) {
  const openid = requireOpenid(env)
  const merchant = env.merchants && env.merchants.findByUserId ? await env.merchants.findByUserId(openid) : null
  if (merchant) {
    return {
...
async function getMyQualification(event, env) {
  ...
  return success({
    qualification,
    qualification_status: qualification ? qualification.qualification_status : QUALIFICATION_STATUS.NOT_SUBMITTED,
...
async function submitQualification(event, env) {
  ...
  const existing = await findOwnedQualification(context, env)
  const normalized = Object.keys(payload).length ? normalizeQualificationPayload(payload) : existing
  if (!normalized) throw serviceError('QUALIFICATION_NOT_FOUND', '请先保存资质草稿')
  ...
  await createOnboardingLog(env, {
    ...
    before_status: existing ? existing.qualification_status : QUALIFICATION_STATUS.NOT_SUBMITTED,
    after_status: QUALIFICATION_STATUS.PENDING_REVIEW,
    operator_role: 'merchant'
  })
  return success({ qualification, onboarding_status: snapshot.onboarding_status || ONBOARDING_STATUS.QUALIFICATION_WAIT })
}
...
async function adminListQualifications(event, env) {
  ...
  if (payload.status) list = list.filter((item) => item.qualification_status === payload.status)
  return success(paginateList(list, payload, { listKey: 'qualifications' }))
}

async function adminGetQualificationDetail(event, env) {
  await requireAdmin(env)
  const id = getPayload(event).qualificationId || getPayload(event).qualification_id
  const qualification = await env.qualifications.findById(id)
  if (!qualification) throw serviceError('QUALIFICATION_NOT_FOUND', '资质记录不存在')
...
async function adminReviewQualification(event, env) {
  ...
  const updated = await env.qualifications.updateById(id, {
    ...
    reject_reason: result === QUALIFICATION_STATUS.REJECTED ? trimText(payload.reason) || '资料不完整' : '',
    supplement_required_fields: result === QUALIFICATION_STATUS.NEED_SUPPLEMENT ? (payload.supplementRequiredFields || payload.supplement_required_fields || []) : [],
  ...
  return success({ qualification: updated, onboarding_status: snapshot.onboarding_status })
}

function getQualificationRequirements() {
  return success({
    mock_warning: '当前为模拟资质认证流程，请勿上传真实身份证、营业执照、保单等敏感材料。',
    subject_types: ['individual', 'business'],
    required_fields: ['subject_type', 'service_categories', 'agreement_checked']
  })
}
