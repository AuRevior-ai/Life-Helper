...
function hasSensitivePaymentContent(fullPath, relativePath, entry) {
  ...
  return (
    ...
    /\b(?:apiv3|api_v3|apiV3)Key\s*[:=]\s*['"][A-Za-z0-9_-]{32,}['"]/i.test(
      content,
    ) ||
    /\bWECHAT_PAY_(?:MCH_ID|API_V3_KEY|PRIVATE_KEY|CERT_SERIAL_NO)\s*=.+/i.test(
      content,
    )
  );
}

function hasRealProjectConfigAppId(fullPath, relativePath, entry) {
  if (entry.isDirectory()) {
    return false;
  }

  const normalized = toPosix(relativePath);
  if (path.basename(normalized) !== "project.config.json") {
    return false;
  }
  ...
  return /^wx[a-f0-9]{16,}$/i.test(`${config.appid || ""}`.trim());
}

const forbiddenMatchers = [
  {
    label: ".git directory",
    test: (relativePath, entry) =>
      relativePath === ".git" && entry.isDirectory(),
  },
  {
    label: "node_modules directory",
    test: (relativePath, entry) =>
      /(^|[\\/])node_modules$/.test(relativePath) && entry.isDirectory(),
  },
  {
    label: "miniprogram_npm directory",
    test: (relativePath, entry) =>
      relativePath === "miniprogram_npm" && entry.isDirectory(),
  },
  {
    label: "env file",
  ...
  {
    label: "private project config",
    test: (relativePath) =>
      path.basename(relativePath) === "project.private.config.json",
  },
  { label: "log file", test: (relativePath) => /\.log$/i.test(relativePath) },
  {
    label: "certificate or private key",
    test: (relativePath) => /\.(pem|key|crt|cer|p12|pfx)$/i.test(relativePath),
  },
  {
    label: "temporary archive",
    test: (relativePath) => /\.(zip|tar|tar\.gz|rar|7z)$/i.test(relativePath),
  },
  {
    label: "test output",
    test: (relativePath) =>
      /(^|[\\/])(coverage|test-results|playwright-report|\.nyc_output)([\\/]|$)/.test(
        relativePath,
      ),
  },
  {
    label: "real payment config",
    test: (relativePath, entry, fullPath) =>
      hasSensitivePaymentContent(fullPath, relativePath, entry),
  },
  {
    label: "real project config AppID",
    test: (relativePath, entry, fullPath) =>
      hasRealProjectConfigAppId(fullPath, relativePath, entry),
  },
];
